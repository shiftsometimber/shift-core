...
test('documented payload persists one draft; no live URL or publication; metadata retained',async()=>{const env=setup();try{const r=await babyLoveRoutes(request({...payload,status:'published'}),env);assert.equal(r.status,200);assert.match(r.headers.get('Content-Type'),/application\/json/);const b=await r.json();assert.equal(b.published,false);assert.equal(b.link,undefined);const row=env.DB.sqlite.prepare('SELECT * FROM knowledge_articles').get();assert.equal(row.status,'draft');assert.equal(row.body,payload.content_markdown);const raw=JSON.parse(env.DB.sqlite.prepare('SELECT payload_json FROM babylove_receipts').get().payload_json);assert.equal(raw.heroImageUrl,payload.heroImageUrl);}finally{env.DB.sqlite.close();}});
test('identical retry succeeds without duplicate; changed content cannot overwrite',async()=>{const env=setup();try{await babyLoveRoutes(request(),env);const retry=await babyLoveRoutes(request(),env);assert.equal((await retry.json()).duplicate,true);assert.equal((await babyLoveRoutes(request({...payload,title:'Changed'}),env)).status,409);assert.equal(env.DB.sqlite.prepare('SELECT count(*) n FROM knowledge_articles').get().n,1);assert.equal(env.DB.sqlite.prepare('SELECT title FROM knowledge_articles').get().title,payload.title);}finally{env.DB.sqlite.close();}});
