...
function trustedImage(raw){try{const u=new URL(String(raw));return u.protocol==='https:'&&!u.username&&!u.password&&!u.port&&IMAGE_HOSTS.has(u.hostname)&&u.pathname.startsWith(IMAGE_PATH_PREFIX)?u:null}catch{return null}}
...
export function representativeImage(row){
 ...
 for(const match of String(row.body||'').matchAll(/!\[([^\]]*)\]\((https?:\/\/[^\s)]+)\)/g)){
  if(trustedImage(match[2]))return {url:new URL(proxiedImage(match[2],row.slug),'https://shiftsometimber.co.uk').href,alt:match[1]};
 }
 return null;
}
export function articleHTML(row){
 ...
 if(author==='SHIFT Team')schema.author={...schema.publisher,name:author};
 if(lead)schema.image=lead.url;
...
export async function dynamicBabyLovePublicRoute(request,env){
 ...
 const body=request.method==='HEAD'?null:articleHTML(row);
 return withArticleResponsePolicy(new Response(body,{headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store, must-revalidate','X-Content-Type-Options':'nosniff','X-Shift-Article-Source':'babylovegrowth','Server-Timing':'article_db;dur='+(loaded-started).toFixed(1)+', article_render;dur='+(performance.now()-loaded).toFixed(1)}}));
}
