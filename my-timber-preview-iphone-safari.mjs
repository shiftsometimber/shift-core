...
async function openToday(page,{reload=false}={}){let lastError;for(let attempt=0;attempt<3;attempt++){if(reload||attempt)await page.reload({waitUntil:'domcontentloaded'});await page.evaluate(attempt=>{document.getElementById('previewAuth').hidden=true;document.getElementById('previewMember').classList.add('is-ready');if(attempt||!document.querySelector('script[src^="/member-my-timber-problem-v1.js"]')){const script=document.createElement('script');script.src=`/member-my-timber-problem-v1.js?v=daily-shift-v2-${Date.now()}`;document.body.appendChild(script)}},attempt);try{await page.locator('[data-life-changed]').waitFor({state:'visible',timeout:20000});return}catch(error){lastError=error}}throw lastError}
...
try{
  ...
  await openToday(page,{reload:true});await page.locator('[data-morning-entry]').click();await page.getByText('On · 8am').waitFor({state:'visible',timeout:15000});pass('Morning entry saves and survives a fresh page load');await screen(page,'06-morning-entry-retained');
  ...
  await page.locator('[data-life-changed]').click();await page.locator('[data-adjust="working_late"]').click();await page.locator('.mt-rebuilt').waitFor({state:'visible',timeout:15000});await pause(page);await screen(page,'08-working-late-rebuilt');
