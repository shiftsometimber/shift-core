...
try{
  ...
  await page.locator('[data-morning-entry]').click();await page.locator('[data-reminder-enabled]').check();await page.locator('[data-reminder-hour]').selectOption('8');await page.locator('[data-reminder-form]').getByRole('button',{name:'Save reminder'}).click();await page.getByText('Morning plan set for 8am.').waitFor({state:'visible',timeout:15000});
  await page.reload({waitUntil:'domcontentloaded'});await page.locator('[data-life-changed]').waitFor({state:'visible',timeout:20000});await page.locator('[data-morning-entry]').click();await page.getByText('On · 8am').waitFor({state:'visible',timeout:15000});pass('Morning entry saves and survives a fresh page load');await screen(page,'06-morning-entry-retained');
