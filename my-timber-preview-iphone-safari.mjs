...
async function auditPalette(page,label){
  const violations=await page.evaluate(()=>{
    ...
    for(const element of document.querySelectorAll('body *')){
      ...
      for(const side of ['Top','Right','Bottom','Left']){const width=parseFloat(style[`border${side}Width`]);const colour=parse(style[`border${side}Color`]);if((!background||background.alpha===0)&&width>0&&style[`border${side}Style`]!=='none'&&colour&&colour.base===resolvedSurface)output.push({kind:`invisible ${side.toLowerCase()} boundary on transparent control`,element:element.tagName,className:String(element.className||'').slice(0,100),colour:colour.base,text:(element.innerText||element.value||'').trim().slice(0,80)})}
