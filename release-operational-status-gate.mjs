...
const required=[
  ...
  "releaseControl.ready&&explicitRelease",
...
for(const phrase of ['fails every release boundary closed','cannot unlock commerce','require explicit sale authorisation','different commit blocks deployment','cannot provide multiple independent approvals','rollback proof blocks deployment']) if(!tests.includes(phrase)) throw new Error(`core proof scenario missing: ${phrase}`);
...
console.log(JSON.stringify({proof:'SHIFT_PHASE_7_RELEASE_CONTROLS',status:'PASS',operationalDomains:ENGINEERING_PROOFS.length,externalGates:EXTERNAL_EVIDENCE.length,coreScenarios:8,commitBinding:'required',approvalSeparation:'required',rollbackReadiness:'required',medicinePurchase:'blocked'},null,2));
