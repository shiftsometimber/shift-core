...
const pub=read('./knowledge-publication-v1.js'),brain=read('./shift-brain-v1.js'),radar=read('./radar-public-v1.js'),fresh=read('./radar-freshness-v2.js'),watch=read('./watchtower-v1.js'),entry=read('./worker-entry-v6.js');
...
must(radar.includes('current:Boolean(freshness.current&&items.length)')&&entry.includes('!body.current')&&entry.includes('strip.remove()'),'stale ticker must fail safe rather than render stale items as current');
