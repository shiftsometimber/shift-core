...
export function newsPublicationDate(row,now=Date.now()) {
  ...
  // Prefer the first successful publication audit over draft/journal metadata.
  // Missing publication metadata stays undated; ingestion/review is not release.
  const value=String(row.first_published_at||content.seo?.datePublished||'');
  ...
  return Number.isFinite(Date.parse(iso))&&Date.parse(iso)<=now?new Date(iso).toISOString():'';
}
