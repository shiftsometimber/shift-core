...
for(const source of health.sources){assert.equal(source.reviewedAt,REVIEWED_AT);assert.ok(source.lastAttemptAt,source.id+' has been attempted');assert.ok(['current','verification_pending','check_delayed','awaiting_review'].includes(source.status));}
...
writeFileSync('medicines-watch-live-proof.json',JSON.stringify({checkedAt:new Date().toISOString(),url:origin+WATCH_PATH,cards:medicines.length,sourceChecks:health.sources.map(({id,status,lastAttemptAt,lastSuccessAt,error})=>({id,status,lastAttemptAt,lastSuccessAt,error})),filters:'pass',treatmentsEntry:'pass',readOnly:'pass'},null,2));
