...
need(/P0_HREF='\/member-p0-v1\.css\?v=4'/.test(shell),'member shell does not inject current versioned P0 CSS');
need(/function ensureP0\(\)/.test(shell)&&/MutationObserver\(\(\)=>ensureP0\(\)\)/.test(shell),'member shell does not retain canonical P0 CSS through member-surface head mutations');
...
need(/GIT_MEMBER_JS_ASSETS=new Set\(\['\/api-adapter-v33d\.js','\/member-product-v33d\.js','\/member-shell-v33g\.js'\]\)/.test(workerEntry),'Worker does not explicitly serve all commissioned Git-authoritative member JS assets');
...
for(const asset of ['member-product-v33d.js','member-shell-v33g.js']){
...
console.log(JSON.stringify({proof:'V1_FRONTEND_SOURCE_P0',status:'PASS',checks:['closed drawer removed from layout geometry without global masking','member hero decoration locally clipped','mobile member sidebar locally contains scroll overflow','mobile member tabs locally contain scroll overflow','canonical v4 P0 stylesheet retained after member-surface head mutation','stale duplicate P0 stylesheet versions removed','notice non-interactive and cleared on successful auth','member dashboard links normalised to extensionless production route','authenticated My Shift points to live dashboard route','401 guard uses live login route','ordinary APIs retain a finite 15-second client budget','Fit generation alone receives the commissioned finite 60-second client budget','member product retains the existing canonical Today API render path','Today API headline renders into the existing Today surface','Today API subhead renders into the existing Today surface','Git-authoritative member product and shell JS served by existing Worker asset binding','member product and shell routes owned by Worker on canonical and www hosts'],authority:'frontend/README.md'},null,2));
