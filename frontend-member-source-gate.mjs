...
need(/member-product-v33d\.js\?v=vision-[4-9]|member-product-v33d\.js\?v=vision-[1-9][0-9]+/.test(preview)&&/member-my-journey-v1\.js\?v=(?:[4-9]|[1-9][0-9]+)/.test(preview),'production member entry does not force corrected Journey and product clients past stale browser caches');
...
need(/my-timber-v11\.js\?v=2/.test(preview),'production member shell does not request the deterministic visible-navigation release');
...
need(/MY_JOURNEY_SRC='\/member-my-journey-v1\.js\?v=(?:[3-9]|[1-9][0-9]+)'/.test(shell),'late member-shell Journey loader can reuse a stale pre-fix client');
...
need(/G2-015-PREMIUM-PLAN-MANAGER/.test(plansPremium)&&/data-plan-manager-v1/.test(plansPremium)&&/data-open-plan/.test(plansPremium)&&/groups\.replaced/.test(plansPremium)&&/location\.assign\(`\/member\/\$\{target\}`\)/.test(plansPremium),'premium My Plans layer does not preserve current/open/history plan-management semantics or route real Grub/Fit plans to their product surfaces');
...
for(const asset of ['/api-adapter-v33d.js','/member-product-v33d.js','/member-shell-v33g.js','/member-progress-v1.js','/member-progress-picture-premium-v1.js','/member-plans-premium-v1.js','/member-plans-premium-v1.css'])need(workerEntry.includes(asset)&&workerEntry.includes('GIT_MEMBER_ASSETS'),`${asset} is not explicitly served as a Git-authoritative member asset`);
need(/path\s*===\s*["']\/shift-me["']/.test(workerEntry)&&/\/member\/dashboard#shiftme/.test(workerEntry),'retired public Shift Me route does not hand off to the authenticated product');
