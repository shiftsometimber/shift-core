...
const rows=all.filter(r=>JSON.parse(r.content_package_json||'{}').destinations?.includes('medicine_news'));
...
console.log(JSON.stringify({articles:rows.length,missing:missing.map(r=>({id:r.id,title:r.headline}))}));
