...
try{
 for(const path of paths){
  ...
  const ir=await fetch(origin+image.pathname+image.search,{signal:AbortSignal.timeout(25000)});assert.equal(ir.status,200,path+' sharing image');assert.match(ir.headers.get('content-type'),/^image\//);
  report.checks.push({path,status:'pass',main_content_preserved:origin!==production,datePublished:data.datePublished,sharing_image:image.pathname,server_timing:response.headers.get('server-timing')});
 }
