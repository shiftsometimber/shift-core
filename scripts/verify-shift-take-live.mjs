...
await Promise.all(Array.from({length:4},async()=>{while(pending.length){
 ...
 results.push({path:item.path,status:response.status,oneShiftTake:!item.originalPublisherNotice,originalPublisherNotice:Boolean(item.originalPublisherNotice),publicationDate:article.datePublished});
}}));
