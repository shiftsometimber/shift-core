...
const SCOPES=new Set(['today','for_a_while','durable','safety']);
...
async function saveFeedbackResponse(request,env,userId,product,body){
  ...
  const scope=normaliseScope(body.scope,sentiment),expiresAt=scopeExpiry(scope,body.expires_at),favourite=sentiment==='yay'&&Boolean(body.favourite);
  await saveFeedback(env,userId,product,entityId,sentiment,body.reason,{...(body.context||{}),scope,favourite},scope,expiresAt,favourite);
  return json({ok:true,product,entity_id:entityId,sentiment,scope,expires_at:expiresAt,favourite},200,request);
}
...
async function replaceWithLearning(request,env,ctx,userId,product,body){
  ...
  if(current){const scope=normaliseScope(body.scope,'nay'),expiresAt=scopeExpiry(scope,body.expires_at);await saveFeedback(env,userId,product,current,'nay',body.reason,{source:'replacement',scope},scope,expiresAt,false);}
...
async function planWithLearning(request,env,ctx,userId,product,body){
  ...
  if(product==='grub'){
    ...
    for(const day of payload.plan.days){for(let i=0;i<(day.meals||[]).length;i++){const meal=day.meals[i];if(!historical.includes(meal.id))continue;const r=await memberProductV4Routes(cloneJsonRequest(request,{type:meal.type,exclude:[...historical,...allUsed],preferences:body.preferences,dislikes:body.dislikes,dietaryRequirements:body.dietaryRequirements}),env,ctx);const j=await r.json().catch(()=>null);if(j?.meal){day.meals[i]=j.meal;allUsed.add(j.meal.id);changed=true;}}day.totals={kcal:(day.meals||[]).reduce((a,m)=>a+Number(m.kcal||m.nutrition?.kcal||0),0),protein_g:(day.meals||[]).reduce((a,m)=>a+Number(m.protein||m.nutrition?.protein_g||0),0)};}
  }else{
    for(const session of payload.plan.sessions||[]){for(let i=0;i<(session.exercises||[]).length;i++){const ex=session.exercises[i];if(!historical.includes(ex.id))continue;const r=await memberProductV4Routes(cloneJsonRequest(request,{group:ex.group,exclude:historical,location:body.location,equipment:body.equipment,limitations:body.limitations,preferences:body.preferences}),env,ctx);const j=await r.json().catch(()=>null);if(j?.exercise){session.exercises[i]=j.exercise;changed=true;}}session.estimated_minutes=(session.exercises||[]).reduce((a,x)=>a+Number(x.minutes||0),0);}
  }
  payload.plan.learning={durable_feedback:true,active_nays_applied:historical.length,temporal_scope:true};
  if(changed)await replaceLatestPlan(env,userId,product,payload.plan);
  return json(payload,200,request);
}

async function saveFeedback(env,userId,product,entityId,sentiment,reason,context,scope='durable',expiresAt=null,favourite=false){await env.DB.prepare(`INSERT INTO product_feedback(user_id,product,entity_id,sentiment,reason,context_json,scope,expires_at,favourite,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP) ON CONFLICT(user_id,product,entity_id) DO UPDATE SET sentiment=excluded.sentiment,reason=excluded.reason,context_json=excluded.context_json,scope=excluded.scope,expires_at=excluded.expires_at,favourite=excluded.favourite,updated_at=CURRENT_TIMESTAMP`).bind(userId,product,entityId,sentiment,clean(reason,500),JSON.stringify(context||{}),scope,expiresAt,favourite?1:0).run();}
async function negativeIds(env,userId,product){const {results}=await env.DB.prepare(`SELECT entity_id FROM product_feedback WHERE user_id=? AND product=? AND sentiment='nay' AND (expires_at IS NULL OR expires_at>CURRENT_TIMESTAMP) ORDER BY updated_at DESC LIMIT 250`).bind(userId,product).all();return (results||[]).map(r=>String(r.entity_id));}
...
async function ensureFeedbackSchema(DB){await DB.exec(`CREATE TABLE IF NOT EXISTS product_feedback (id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,product TEXT NOT NULL,entity_id TEXT NOT NULL,sentiment TEXT NOT NULL CHECK(sentiment IN ('yay','nay')),reason TEXT,context_json TEXT NOT NULL DEFAULT '{}',scope TEXT NOT NULL DEFAULT 'durable',expires_at TEXT,favourite INTEGER NOT NULL DEFAULT 0,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(user_id,product,entity_id)); CREATE INDEX IF NOT EXISTS idx_product_feedback_user_product ON product_feedback(user_id,product,sentiment,updated_at);`);for(const sql of [`ALTER TABLE product_feedback ADD COLUMN scope TEXT NOT NULL DEFAULT 'durable'`,`ALTER TABLE product_feedback ADD COLUMN expires_at TEXT`,`ALTER TABLE product_feedback ADD COLUMN favourite INTEGER NOT NULL DEFAULT 0`]){try{await DB.exec(sql)}catch{}}}
function normaliseScope(v,sentiment){const s=String(v||'').toLowerCase().replace(/[ -]+/g,'_');if(SCOPES.has(s))return s;return sentiment==='nay'?'durable':'durable';}
function scopeExpiry(scope,explicit){if(explicit){const d=new Date(explicit);if(!Number.isNaN(d.getTime()))return d.toISOString();}const now=Date.now();if(scope==='today'){const d=new Date();d.setHours(23,59,59,999);return d.toISOString();}if(scope==='for_a_while')return new Date(now+14*86400000).toISOString();return null;}
