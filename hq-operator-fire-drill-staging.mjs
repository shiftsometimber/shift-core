...
try{
  ...
  const radarSeed=await DB.prepare(`INSERT INTO radar_events(event_key,status,headline,content_package_json,reviewed_at) VALUES('b06-healthy-baseline','published','B06 healthy baseline','{"destinations":["ticker_knowledge"]}',CURRENT_TIMESTAMP)`).run();
