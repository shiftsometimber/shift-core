...
const recipeBatches=[...GRUB_BATCH_001,...GRUB_BATCH_002],exerciseBatches=[...FIT_BATCH_001,...FIT_BATCH_002];
const authoredRecipes=recipeBatches.map(x=>({id:x.id,mealType:x.data.mealType,title:x.title,source:'structured-draft'}));
const authoredExercises=exerciseBatches.map(x=>({id:x.id,title:x.title,movementPattern:x.data.movementPattern,equipment:x.data.equipment,locations:x.data.locations,source:'structured-review'}));
...
function grubSim(days,pool){
  ...
  const firstRepeat={};for(const mt of ['breakfast','lunch','dinner','snack']){const p=sequence.filter(x=>x.mealType===mt);const seen=new Set();const idx=p.findIndex(x=>seen.has(x.id)||!seen.add(x.id));firstRepeat[mt]=idx<0?null:idx+1;}
...
const recipeValidation=recipeBatches.map(validateContentItem),exerciseValidation=exerciseBatches.map(validateContentItem);
