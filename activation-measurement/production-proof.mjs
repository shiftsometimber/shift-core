...
try{
 ...
 for(const path of ['/v1/hq/journey','/v1/me']){
 ...
 const execute=(sql,args)=>{
  if(!/^(SELECT|WITH)\b/.test(sql)||sql.includes(';'))throw Error('Only one read-only aggregate statement is allowed');
 ...
 report.activation=await activationScorecard(DB,{days:30,now:report.checkedAt});assert.equal(report.activation.available,true,'Live account evidence must be available');
 report.pass=true;
