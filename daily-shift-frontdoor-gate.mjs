...
for(const marker of ['getDailyShift','adjustDailyShift','logHydration','data-adjust','data-water'])need(ui.includes(marker),`real Today action missing: ${marker}`);
for(const marker of ['getDailyShift','adjustDailyShift'])need(api.includes(marker),`Today API adapter missing: ${marker}`);
for(const marker of ['/v1/shift/daily-plan','/v1/shift/daily-adjust','composeDailyOutput','working_late','shift_plans','daily_shift_rebuilt'])need(backend.includes(marker),`joined Today contract missing: ${marker}`);
