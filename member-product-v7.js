...
function rankRecipes(rows,likes){if(!likes.length)return rows;return rows.map((row,index)=>{const haystack=`${row.title} ${(row.data?.tags||[]).join(' ')} ${(row.data?.ingredients||[]).map(x=>x.item).join(' ')}`.toLowerCase();return{row,index,score:likes.reduce((n,x)=>n+(haystack.includes(x)?3:0),0)}}).sort((a,b)=>b.score-a.score||a.index-b.index).map(x=>x.row)}
