...
export async function memberJourneySnapshot(DB,options={}){
 const window=reportingWindow(options),activation=await activationScorecard(DB,options);
 const table=await DB.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='product_events'").first();
 if(!table||!activation.available)return {windowDays:window.days,asOf:window.asOf,steps:[],transitions:[],activation,privacy:'aggregate_only',interpretation:'Event evidence unavailable; no conversion rate inferred.'};
