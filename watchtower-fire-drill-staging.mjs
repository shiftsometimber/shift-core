...
try{
 ...
 // The incident must be retained after the endpoint recovers, not disappear from operator history.
 mode='green';
 ...
 assert.ok(rows.results.at(-1)?.ok===1||Number(rows.results.at(-1)?.ok)===1,'latest recovered sample must be healthy');
 assert.ok(['GREEN','AMBER','RED'].includes(restoredButRemembered.status));

 // Simulate the retained incident ageing outside the 24h trend window; current probes are healthy.
 ...
 console.log(JSON.stringify({
   ...
   degraded:{status:degraded.status,alert:latency.code,nextAction:latency.nextAction},
   outage:{status:outage.status,alert:failure.code,nextAction:failure.nextAction},
   retainedHistory:{coreSamples:rows.results.length,failedSamples:rows.results.filter(x=>!Number(x.ok)).length,latestHealthy:!!Number(rows.results.at(-1)?.ok)},
   recovery:{status:recovered.status,currentHealthy:recovered.platform.probes.every(x=>x.ok)}
 },null,2));
 console.log('PASS B07 Watchtower fire drill: GREEN -> latency AMBER -> outage RED -> retained probe history/operator action -> healthy recovery -> GREEN after incident window.');
