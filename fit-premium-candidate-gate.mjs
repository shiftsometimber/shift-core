...
const visibleText=s=>decode(String(s).replace(/<title\b[^>]*>[\s\S]*?<\/title>/gi,' ').replace(/<desc\b[^>]*>[\s\S]*?<\/desc>/gi,' ').replace(/<[^>]+>/g,' ')).replace(/\s+/g,' ').trim();
...
need(contracts.length===26,'render contract count must remain 26');
const genericCue=/stable safe setup|controlled intended path|controlled return\/end/i;
for(const row of rows){
  const file=String(row.asset||''),contract=contractById.get(row.canonical_movement);
  need(!!contract,`${row.canonical_movement}: render contract missing`);
  ...
  const s=fs.readFileSync(file,'utf8'),text=visibleText(s);
  ...
  if(contract){
    need(text.includes(norm(contract.instruction)),`${row.canonical_movement}: visible instruction truncated or altered`);
    for(const key of ['start','move','finish'])need(text.includes(norm(contract[key])),`${row.canonical_movement}: visible ${key.toUpperCase()} cue truncated or altered`);
  }
...
if(fail.length){console.error(JSON.stringify({proof:'FIT_PREMIUM_CANDIDATE_TECHNICAL_QA_V2',status:'FAIL',fail},null,2));process.exit(1)}
console.log(JSON.stringify({proof:'FIT_PREMIUM_CANDIDATE_TECHNICAL_QA_V2',status:'PASS',produced:rows.length,fullCueRendering:true,technicalCandidates:rows.map(x=>x.canonical_movement),criterion:'Self-contained movement-bound accessible START -> MOVE -> FINISH premium candidates with complete untruncated movement-specific state cues. Domain/anatomical/member-comprehension approval remains separate.'},null,2));
