...
console.log(JSON.stringify({proof:'FIT_PREMIUM_CANDIDATE_TECHNICAL_QA_V1',status:'PASS',produced:rows.length,technicalCandidates:rows.map(x=>x.canonical_movement),criterion:'Self-contained movement-bound accessible START -> MOVE -> FINISH premium candidates only. Domain/anatomical/member-comprehension approval remains separate.'},null,2));
