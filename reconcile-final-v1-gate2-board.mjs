...
const visualPath=path.resolve(process.env.FINAL_V1_FIT_ASSET_REPORT||p('final-v1-production-serving-evidence/fit-premium-assets/fit-v1-26-premium-assets-production.json'));
...
const servingDigest=digest(servingPath),publicationDigest=digest(publicationPath),visualDigest=digest(visualPath);
...
need(visual?.proof==='FIT_V1_26_PREMIUM_ASSETS_PRODUCTION_HTTP_V1'&&visual?.status==='PASS','wrong Fit premium production visual proof');
need(visual?.accepted===26&&visual?.served===26&&visual?.allThreeStates===true,'all 26 accepted premium Fit visuals were not production-served');
need(Array.isArray(visual?.rows)&&visual.rows.length===26&&new Set(visual.rows.map(x=>x.id)).size===26&&visual.rows.every(x=>x.ok===true),'Fit premium visual row proof incomplete');
