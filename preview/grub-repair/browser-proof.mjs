...
try{for(const [index,[name,engine,viewport,style]]of matrix.entries()){
 const browser=await engine.launch(),ctx=await browser.newContext({viewport}),page=await ctx.newPage(),row={name,style,checks:[]};report.cases.push(row);
 ...
 }catch(e){row.pass=false;row.error=String(e.stack).replaceAll(fixture.password,'[redacted]');await page.screenshot({path:dir+'/'+name+'-failure.png',fullPage:true});throw e}finally{await ctx.close();await browser.close();writeFileSync(dir+'/report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(row))}
