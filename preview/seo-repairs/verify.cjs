...
const base=process.argv[2]||'https://shift-seo-repair-preview.matobrien.workers.dev';
...
const checks=[];const failures=[];fs.mkdirSync('seo-repair-proof/screenshots',{recursive:true});
...
(async()=>{
  ...
  for(const path of paths){
   ...
   try{
    ...
    row.dom=await page.evaluate(()=>{
     ...
     return {title:document.title,canonical:[...document.querySelectorAll('link[rel=canonical]')].map(x=>x.href),h1:[...document.querySelectorAll('h1')].map(x=>({text:x.textContent.trim(),visible:visible(x)})),mainText:main?.innerText,bodyText:document.body.innerText,viewport:innerWidth,scrollWidth:document.documentElement.scrollWidth,images:[...document.images].filter(visible).map(x=>({src:x.currentSrc,alt:x.getAttribute('alt'),loaded:x.complete&&x.naturalWidth>0,rect:{width:x.getBoundingClientRect().width,height:x.getBoundingClientRect().height},loading:x.loading})),primaryLinks:[...document.querySelectorAll('.desktop-nav a')].map(x=>({href:x.getAttribute('href'),text:x.textContent.trim()})),goodToTalkLinks:[...document.querySelectorAll('a')].filter(x=>x.textContent.trim()==='Good to Talk').map(x=>x.getAttribute('href')),cookieChoices:!!document.querySelector('#sstCookieSettings'),gtmScripts:[...document.scripts].filter(x=>x.src.includes('googletagmanager.com/gtm.js')).map(x=>x.src),schema,ticker:!!document.querySelector('#shift-public-news')};
    });
    record(key+' status',row.status===200,row.status);record(key+' preview noindex',/noindex/.test(row.headers['x-robots-tag']||''),row.headers['x-robots-tag']);
    record(key+' source identity',row.headers['x-shift-preview-commit']===process.env.CANDIDATE_SHA,row.headers['x-shift-preview-commit']);
    ...
    if(primary.includes(path)){
     ...
     const file=(engine===webkit?'webkit':'chromium')+'-'+width+'-'+(path==='/'?'home':path.replace(/\W+/g,'_'))+'.png';await page.screenshot({path:'seo-repair-proof/screenshots/'+file});row.screenshot=file;
    }
    if(width===390&&engine===chromium&&['/medicine-news/glp-1-analogs-research-update','/medicine-news/glp-1-nutritional-paradox','/mens-mental-health'].includes(path)){const file='chromium-390-'+path.replace(/\W+/g,'_')+'.png';await page.screenshot({path:'seo-repair-proof/screenshots/'+file,fullPage:true});row.screenshot=file;}
   }catch(e){row.error=String(e);record(key+' execution',false,String(e));}
   row.pageErrors=errors;row.blockedRequests=blocked;snapshots.push(row);await context.close();
  }
  await browser.close();
 }
 ...
 const robot=await fetch(base+'/robots.txt');record('Preview robots disallow',/Disallow: \//.test(await robot.text()),null);
 const denied=await fetch(base+'/v1/profile',{method:'POST'});record('Preview rejects writes',denied.status===405,denied.status);
 fs.writeFileSync('seo-repair-proof/browser.json',JSON.stringify({checkedAt:new Date().toISOString(),base,candidateCommit:process.env.CANDIDATE_SHA,scope:'Read-only public rendering and navigation. Analytics delivery blocked. Not authenticated journeys, physical Safari or clinical certification.',checks,snapshots,failures},null,2));
 console.log(JSON.stringify({contexts:snapshots.length,checks:checks.length,failed:failures.length,failures},null,2));
 if(failures.length)process.exitCode=1;
})().catch(e=>{fs.writeFileSync('seo-repair-proof/browser-fatal.txt',String(e.stack||e));console.error(e);process.exitCode=1});
