...
export async function b1PreviewRoutes(request,env,ctx){
 ...
 if(['/reset-password','/reset-password.html','/member-login.html'].includes(p)&&request.method==='GET'){
  ...
  let body=repairPasswordResetDocument(await r.text());body=body.replace(/https:\/\/api\.shiftsometimber\.co\.uk/g,u.origin).replace(/(<head\b[^>]*>)/i,'$1<script>window.SST_API_BASE=location.origin;</script>').replace(/(<body\b[^>]*>)/i,'$1<aside style="padding:12px;background:#e7e3da;color:#050505">B1 preview — fictional account only. Your live password is unchanged.</aside>');
