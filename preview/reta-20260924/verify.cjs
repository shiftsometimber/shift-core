...
for(const [engine,name] of [[chromium,'chromium'],[webkit,'webkit']])for(const width of [390,1440]){
 ...
 for(const mode of live?['candidate']:['baseline','candidate']){
  ...
  if(mode==='baseline')original=state;
   ...
   if(original){assert.deepEqual(state.styles,original.styles);assert.deepEqual(state.mainStyles,original.mainStyles);assert.deepEqual(state.header,original.header);assert.deepEqual(state.h1,original.h1);assert.equal(state.main,original.main)}
