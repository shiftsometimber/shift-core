...
for(const [name,engine,viewport]of matrix){
 ...
 const mark=phase=>{row.phase=phase;writeFileSync(dir+'/report.json',JSON.stringify(report,null,2));console.log(JSON.stringify({name,phase,at:new Date().toISOString()}))};
 const deadline=setTimeout(()=>{row.deadlineExceeded=true;context.close().catch(()=>{});browser.close().catch(()=>{})},180000);
 row.telemetry=[];page.on('requestfailed',r=>{if(new URL(r.url()).pathname==='/v1/events')row.telemetry.push({type:'failed',error:r.failure()?.errorText})});page.on('response',r=>{if(new URL(r.url()).pathname==='/v1/events')row.telemetry.push({type:'response',status:r.status()})});
 mark('login');
 try{
  ...
  mark('settings');await page.goto(origin+'/member/settings');
  ...
  mark('retained-fit');const retained=await json('/v1/fit/activity');
  ...
  mark('fit-rebuild');const oldList=(await json('/v1/plan/list')).plans,currentId=oldList.current.find(p=>p.type==='fit').id;
  ...
  mark('historical-export');const exported=await context.request.post(origin+'/v1/privacy/export',{headers:{Origin:origin},data:{}});assert(exported.ok());const records=await exported.json();assert(records.savedPlans.some(p=>p.id===currentId&&JSON.stringify(JSON.parse(p.plan_json))===JSON.stringify(retained.plan)),'Original plan remains unchanged in export');
  mark('rebuilt-persistence');await page.reload();await page.locator('.sf-current-step').first().waitFor();assert.deepEqual(await json('/v1/fit/activity'),rebuilt);
  await context.request.post(origin+'/v1/auth/logout',{headers:{Origin:origin},data:{}});await login();mark('rebuilt-persistence');await page.reload();await page.locator('.sf-current-step').first().waitFor();assert.deepEqual(await json('/v1/fit/activity'),rebuilt);
 ...
 }catch(e){row.status='fail';row.pageState=await page.evaluate(()=>({session:document.body.dataset.memberSession,visibility:document.visibilityState,todayClass:document.getElementById('panel-today')?.className,hasExposureScript:[...document.scripts].some(s=>s.src.includes('continuity-exposure'))})).catch(()=>null);row.navigation=await page.locator('.sst-member-tabs a').evaluateAll(nodes=>nodes.map(a=>({label:a.textContent,href:a.getAttribute('href')}))).catch(()=>[]);row.error=String(e.stack).replaceAll(fixture.password,'[redacted]');await page.screenshot({path:dir+'/'+name+'-failure.png',fullPage:true}).catch(()=>{});}
 finally{clearTimeout(deadline);console.log(JSON.stringify({name,status:row.status,phase:row.phase,error:row.error}));await context.close();await browser.close();writeFileSync(dir+'/report.json',JSON.stringify(report,null,2))}
}
