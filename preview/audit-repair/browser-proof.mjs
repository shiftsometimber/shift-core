...
for(const [name,engine,viewport]of matrix){
 ...
 try{
  ...
  const link=page.locator('a[href="/member/dashboard#visualise"]').first();await link.click();await page.locator('#panel-visualise.active').waitFor();
 ...
 }catch(e){row.status='fail';row.error=String(e.stack).replaceAll(fixture.password,'[redacted]');await page.screenshot({path:dir+'/'+name+'-failure.png',fullPage:true}).catch(()=>{});}
