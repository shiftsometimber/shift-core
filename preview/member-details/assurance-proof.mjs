...
async function guarded(browser,viewport,stored=false){
 const context=await browser.newContext({viewport,serviceWorkers:'block'}),trace={gtm:[],telemetry:[],technicalAssets:[],blockedWrites:[],blockedExternal:[]};let phase='before-consent';
 ...
 await context.route('**/*',async route=>{const r=route.request(),u=new URL(r.url());
  if(u.hostname==='static.cloudflareinsights.com'&&r.method()==='GET'){trace.technicalAssets.push({phase,url:r.url()});return route.continue();}
  if(tracker(u)){trace.telemetry.push({phase,provider:(u.hostname.endsWith('cloudflareinsights.com')||u.pathname==='/cdn-cgi/rum')?'Cloudflare':'Google-or-other',url:r.url(),method:r.method(),body:r.postData()});return route.fulfill({status:204,body:''});}
...
for(const[name,type,viewport]of configs){const browser=await type.launch({headless:true});
 await check(name+' Google cookie-choice journey; Cloudflare recorded separately',async()=>{const g=await guarded(browser,viewport),page=await g.context.newPage(),errors=[];page.on('pageerror',e=>errors.push(String(e)));try{
  ...
  const before=await state(page);assert.equal(g.trace.gtm.length,0,'tag manager loaded before consent');assert.equal(g.trace.telemetry.filter(x=>x.provider!=='Cloudflare').length,0,'Google/other collection attempted before consent');assert.equal(before.disabled,true);assert(before.settings);
 ...
 if(name.endsWith('desktop'))for(const path of ['/member-login','/start-here','/shift-health','/programme?token=FICTIONAL_PRIVACY_CANARY','/programme#FICTIONAL_PRIVACY_CANARY'])await check(name+' suppress '+path,async()=>{const g=await guarded(browser,viewport,true),page=await g.context.newPage();try{const r=await page.goto(LIVE+path,{waitUntil:'domcontentloaded',timeout:45000});await page.waitForTimeout(1000);const s=await state(page);assert.equal(g.trace.gtm.length,0,'private route attempted Google load');assert.equal(g.trace.telemetry.filter(x=>x.provider!=='Cloudflare').length,0,'private route attempted Google/other collection');assert.equal(s.suppressed,true);assert(!JSON.stringify(g.trace.telemetry).includes('FICTIONAL_PRIVACY_CANARY'),'URL canary appears in attempted analytics payload');return {url:page.url(),status:r.status(),state:s,trace:g.trace};}finally{writeFileSync(dir+'/'+name+'-suppress-'+sha(path).slice(0,10)+'.json',JSON.stringify({path,trace:g.trace},null,2));await g.context.close();}});
 ...
 if(name.startsWith('chromium'))for(const path of ['/','/programme','/shift-health','/shift-newsroom','/about','/member-login'])await check(name+' performance '+path,async()=>{const g=await guarded(browser,viewport),page=await g.context.newPage();try{
  await page.addInitScript(()=>{window.__shiftLab={lcp:null,cls:0,shifts:[]};try{new PerformanceObserver(l=>{for(const e of l.getEntries())window.__shiftLab.lcp=e.startTime}).observe({type:'largest-contentful-paint',buffered:true});new PerformanceObserver(l=>{for(const e of l.getEntries())if(!e.hadRecentInput){window.__shiftLab.cls+=e.value;window.__shiftLab.shifts.push({value:e.value,time:e.startTime,sources:(e.sources||[]).map(s=>({tag:s.node?.tagName,id:s.node?.id,classes:s.node?.className,previous:s.previousRect,current:s.currentRect}))});}}).observe({type:'layout-shift',buffered:true});}catch{}});
  const r=await page.goto(LIVE+path,{waitUntil:'domcontentloaded',timeout:45000});await page.waitForTimeout(2000);const metrics=await page.evaluate(()=>{const n=performance.getEntriesByType('navigation')[0];return {url:location.href,title:document.title,h1:document.querySelectorAll('h1').length,viewport:innerWidth,documentWidth:document.documentElement.scrollWidth,ttfbMs:n.responseStart-n.startTime,domContentLoadedMs:n.domContentLoadedEventEnd,lcpMs:window.__shiftLab.lcp,cls:window.__shiftLab.cls,shifts:window.__shiftLab.shifts,transferBytes:n.transferSize,resourceCount:performance.getEntriesByType('resource').length};});metrics.name=name;metrics.status=r.status();report.performance.push(metrics);assert.equal(r.status(),200);if(['/programme','/member-login'].includes(path))await page.screenshot({path:dir+'/'+name+path.replaceAll('/','-')+'-layout.png',fullPage:false});return metrics;
...
await check('Real preview data lifecycle, account isolation, health erasure and bounded reads',async()=>{
 ...
 try{const meta=(await api(a,'/__preview/meta')).body;assert.equal(meta.source,process.env.PREVIEW_SOURCE_SHA);assert.equal(meta.productionBindings,false);
  const fixtures=JSON.parse(readFileSync('work/staging/generated/probe.json','utf8'));for(const[c,index]of[[a,0],[b,1]])await api(c,'/v1/auth/login','POST',{email:'probe'+fixtures.ids[index]+'@example.invalid',password:fixtures.password});
