import {resumeFictionalPreview} from '../../work/staging/account-capacity.mjs';
...
export default {async fetch(request,env,ctx){
 ...
 if(path==='/__preview/start'&&request.method==='POST'){
  ...
  try{const resumed=await resumeFictionalPreview(request,env,authenticateMember);if(resumed)return resumed;}catch{return page(intro(env,'The preview could not check your existing session. Please retry without creating another account.'),503);}
