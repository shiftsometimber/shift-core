...
for(const [name,engine,size]of [['chromium-desktop',chromium,{width:1365,height:900}],['chromium-phone',chromium,{width:390,height:844}],['webkit-desktop',webkit,{width:1365,height:900}],['webkit-phone',webkit,{width:390,height:844}]]){
 ...
 try{
  ...
  await page.locator('[name="gpPractice"]').focus();await page.keyboard.press('Tab');row.checks.push('Native labelled fields and directory selector remain keyboard reachable');
  assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));assert.deepEqual(external,[]);assert.deepEqual(row.errors,[]);
