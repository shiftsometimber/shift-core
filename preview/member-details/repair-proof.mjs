...
async function context(browser,width,candidate,{stored=false,slowSession=false}={}){
 ...
 await context.addInitScript(()=>{window.__repair={shifts:[],withdrawalAt:null};try{new PerformanceObserver(l=>{for(const e of l.getEntries())if(!e.hadRecentInput)window.__repair.shifts.push({value:e.value,time:e.startTime,nodes:(e.sources||[]).map(s=>({id:s.node?.id,cls:s.node?.className,previous:s.previousRect,current:s.currentRect}))});}).observe({type:'layout-shift',buffered:true});}catch{}window.addEventListener('sst:analytics-consent',e=>{if(e.detail?.analytics===false)window.__repair.withdrawalAt=Date.now()});});
