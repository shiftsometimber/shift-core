...
export function approvedFitPack(pack=JSON.parse(fs.readFileSync(root+'/approval.json'))){
 assert.equal(pack.records.length,300);
 assert.equal(new Set(pack.records.map(r=>r.id)).size,300);
 assert.equal(pack.records.filter(r=>r.status==='approved').length,pack.approvedImages);
 assert.equal(pack.records.filter(r=>r.status==='held').length,pack.heldImages);
 assert.equal(pack.approvedImages+pack.heldImages,300);
 assert.equal(pack.records.filter(r=>r.status==='approved').reduce((n,r)=>n+r.variants.length,0),pack.approvedImageVariantRows);
 ...
 for(const r of pack.records){
  ...
  if(r.approvalMethod==='delegated AI visual correction review'){
   assert.equal(r.correctionReview?.status,'pass',`Correction review missing: ${r.id}`);
   assert.ok(r.correctionReview.observation&&r.correctionReview.originalHoldReason);
   assert.equal(r.correctionReview.sha256,r.sha256);
   assert.notEqual(r.correctionReview.originalRejectedSha256,r.sha256);
  }
...
export function bindApprovedFit(records,pack=approvedFitPack()){
 ...
 return records.map(r=>{
  ...
  return {...r,status:'approved-visual',image:{url:m.image,alt:`${r.title} — three-panel movement illustration`,note:(m.correctionReview?'Image corrected and visually checked by Codex at Matt’s request.':'Visual approved by Matt.')+' Base movement illustration; use the original written instructions for your selected variant.',scope:'canonical-example',productionApproved:false,sourceHash:r.sourceHash,imageHash:m.sha256}};
 });
}
export function fitV3Assets(){
 ...
 const html=`<!doctype html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>SHIFT Fit · Movement visual catalogue</title><link rel="stylesheet" href="/catalogue.css"><style>
