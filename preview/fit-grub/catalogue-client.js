(()=>{
  ...
  const labelled=object=>Object.entries(object||{}).map(([key,value])=>`<p><strong>${esc(key.replaceAll('_',' ').replace(/^./,c=>c.toUpperCase()))}:</strong> ${esc(Array.isArray(value)?value.map(x=>String(x).replaceAll('-',' ')).join(', ')||'None listed':value)}</p>`).join('');
  ...
  function variant(r,index){
    ...
    $('#variant-content').innerHTML=`<h3>${esc(v.title)}</h3><p>Original protocol instructions</p>${list(v.instructions,true)}<dl>${Object.entries(v.dosage||{}).map(([k,v])=>`<dt>${esc(k.replaceAll('_',' '))}</dt><dd>${esc(v)}</dd>`).join('')}</dl><p>Equipment: ${esc([v.equipment].flat().join(', '))}</p><h3>Recorded limitations</h3>${labelled(v.limitations)}<p class="notice">${r.image?'The image above illustrates the base movement only.':'No matching illustration has been added.'} This variant’s image match is pending; follow its specific instructions, equipment and dosage.</p>`;
  }
  function open(r,button){
    opener=button;const s=r.source;
    $('#detail-content').innerHTML=`<p class="eyebrow">${esc(r.meal||'SHIFT Fit')}</p><h2 id="detail-title">${esc(r.title)}</h2>${photo(r)}${r.image?`<p class="notice">${esc(r.image.note)}</p>`:'<p class="notice">A matching photograph has not yet been added.</p>'}${r.kind==='food'?`<p>Serves ${esc(s.servings)}</p><h3>Ingredients</h3>${list(s.ingredients.map(x=>x.amount+' '+x.item))}<h3>Method</h3>${list(s.method,true)}<h3>Equipment</h3>${list([s.equipment].flat().filter(Boolean))}<h3>Storage</h3>${typeof s.storage==='string'?'<p>'+esc(s.storage)+'</p>':s.storage?labelled(s.storage):'<p>No storage guidance supplied.</p>'}<h3>Food safety</h3>${list(s.food_safety)}<h3>Allergens</h3>${list(s.allergens)}`:`<h3>Base movement instructions</h3>${list(s.instructions,true)}<h3>Form and safety cues</h3>${list([...(s.form_cues||[]),...(s.safety_cues||[])])}<p><strong>${esc(s.regression?.label)}:</strong> ${esc(s.regression?.instruction)}</p><label class="variant-label">Inspect a variant<select id="variant">${r.variants.map((v,i)=>`<option value="${i}">${esc(v.title)}</option>`).join('')}</select></label><section id="variant-content"></section>`}`;
