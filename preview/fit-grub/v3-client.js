(()=>{
 ...
 function render(){
  ...
  $('#fit-count').textContent=`${found.length} movements · ${status==='held'?'Images withheld':status==='approved'?'Images ready':'All statuses'}`;
  $('#fit-cards').innerHTML=found.slice(page*pageSize,(page+1)*pageSize).map(r=>r.status==='approved'?`<article class="fit-card"><img src="${r.image}" width="${r.width}" height="${r.height}" alt="${esc(r.title)} — three-panel illustration" loading="lazy" decoding="async"><div class="copy"><p class="eyebrow">Image ready</p><h2>${esc(r.title)}</h2><p>${esc(r.guide.category)} · ${esc(r.guide.floorAccess)}</p><p>${r.variants.length} source protocols with written guidance</p><button type="button" data-open-fit="${r.id}">View ${esc(r.title)}</button></div></article>`:`<article class="fit-card held"><p class="eyebrow">Held · image excluded</p><h2>${esc(r.title)}</h2><p>${esc(r.reason)}</p><button type="button" data-open-fit="${r.id}">View ${esc(r.title)} guidance</button></article>`).join('')||'<p>No movements found. Try another search.</p>';
 ...
 $('#fit-cards').addEventListener('click',e=>{
  ...
  opener=button;activeGuide=r.guide;$('#fit-detail').innerHTML=`<p class="eyebrow">SHIFT Fit · ${r.status==='approved'?'Image ready':'Image held'}</p><h2 id="fit-title">${esc(r.title)}</h2>${r.image?`<img class="fit-large" src="${r.image}" width="${r.width}" height="${r.height}" alt="${esc(r.title)} — all three panels"><div class="fit-panels">${[0,1,2].map(i=>`<figure><div class="fit-panel" style="aspect-ratio:${r.width}/${r.height*3}"><img src="${r.image}" style="left:-${i*100}%" alt="${esc(r.title)} — panel ${i+1}"></div><figcaption>Panel ${i+1} of 3</figcaption></figure>`).join('')}</div>`:''}${instructions(r)}<p>Use alongside the written instructions for the exact exercise in your programme.</p><details><summary>${r.variants.length} mapped variant records</summary><ul>${r.variants.map(v=>`<li>${esc(v.name)}</li>`).join('')}</ul><p>These are source mappings. A shared image does not demonstrate every variation’s technique or equipment.</p></details>`;
