...
for(let index=0;index<checks.length;index++){
 ...
 try{
  ...
  for(const path of ['/shift-health','/mental-health/getting-professional-help','/member-login','/life-back']){
   ...
   await page.locator('#site-drawer .drawer-close').click();await page.locator('#site-drawer').waitFor({state:'hidden'});await geometry(page);const footerSizing=await page.locator('.site-footer').evaluate(el=>({font:getComputedStyle(el.querySelector('h2')).fontSize,columns:getComputedStyle(el.querySelector('.footer-grid')).gridTemplateColumns.split(' ').length}));assert.equal(footerSizing.font,'14px',path+' footer heading consistency');assert.equal(footerSizing.columns,viewport.width>950?6:2,path+' footer column consistency');if(path==='/shift-health'){assert.equal(await page.locator('[data-shift-link-repair] h2').evaluate(el=>getComputedStyle(el).fontSize),'22px','Related-guide heading owns its typography');const related=await page.locator('[data-shift-link-repair]').boundingBox();assert(related.x>=15&&related.x+related.width<=viewport.width-15,'Related guides must stay within the shared page margins');}
