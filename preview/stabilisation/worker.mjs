...
export default {async fetch(request,env,ctx){
 ...
 if(['/v1/contact','/v1/continuity-interest'].includes(path)||(/^\/(?:checkout|payment|treatment-order|treatment-assessment|hq|employer)/.test(path)&&!(env.LAUNCH_REPAIR_PREVIEW==='true'&&path==='/treatment-order'&&request.method==='GET')))return new Response('This action is disabled in the isolated preview. The live service is unchanged.',{status:403,headers:privateHeaders});
