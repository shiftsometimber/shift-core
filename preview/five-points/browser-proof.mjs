...
for(const [index,[name,engine,viewport,topic,outcome]] of matrix.entries()){
 ...
 try{
  ...
  const feedbackSummary=page.locator('#dailyCheckinFollowup summary');await feedbackSummary.waitFor();
  const colours=await feedbackSummary.evaluate(el=>{const panel=el.closest('#dailyCheckinFollowup');return {text:getComputedStyle(el).color,background:getComputedStyle(panel).backgroundColor}});
  const luminance=colour=>colour.match(/[\d.]+/g).slice(0,3).map(Number).map(n=>n/255).map(n=>n<=.04045?n/12.92:((n+.055)/1.055)**2.4).reduce((sum,n,i)=>sum+n*[.2126,.7152,.0722][i],0);
  const fg=luminance(colours.text),bg=luminance(colours.background),contrast=(Math.max(fg,bg)+.05)/(Math.min(fg,bg)+.05);assert(contrast>=4.5,'Today feedback contrast '+contrast);row.checks.push('Today feedback prompt contrast exceeds 4.5:1');
