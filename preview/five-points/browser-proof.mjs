...
for(const [index,[name,engine,viewport,topic,outcome]] of matrix.entries()){
 ...
 try{
  ...
  life=await api(ctx,'/v1/life-back');assert.equal(life.progress.entries.length,before.entries.length);assert.equal((life.progress.shiftHistory?.[0]||life.progress.nextShift).reviews.at(-1).outcome,outcome);
