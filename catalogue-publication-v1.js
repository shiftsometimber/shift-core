...
export async function cataloguePublicationRoute(request,env) {
  if(new URL(request.url).pathname.replace(/\/+$/,'')!=='/v1/commissioning/catalogue-publication')return null;
  ...
  try{return reply({...await publishFixedCatalogue(env.DB,release),workflow_sha:identity.claims.sha})}
