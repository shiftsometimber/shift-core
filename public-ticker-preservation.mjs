// The only response-body change in r3 is these three exact ticker version tokens.
...
export function preserveTickerVersion(input){
 ...
 if(!a&&!w&&!s)return input;
 if(a!==1||w!==1||s!==1)throw Error('Unexpected old ticker version markers');
 html=html.replace(attr,`data-shift-news-ticker="${after}"`).replace(wire,`data-shift-ai-full-wire="${after}"`).replace(script,`src="/assets/public-news-ticker-v1.js?v=${after}"`);
