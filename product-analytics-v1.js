...
export async function recordProductEvent(env,{userId=null,eventName,surface='unknown',properties={},sessionId=null,source='server'}={}){
  ...
  const r=await env.DB.prepare(`INSERT INTO product_events(user_id,event_name,surface,session_id,source,properties_json,occurred_at,created_at) VALUES(?,?,?,?,?,?,?,CURRENT_TIMESTAMP)`).bind(userId,name,String(surface||'unknown').slice(0,80),sessionId?String(sessionId).slice(0,120):null,String(source||'server').slice(0,40),JSON.stringify(cleanProperties),occurredAt).run();
  return{id:Number(r?.meta?.last_row_id||0),event_name:name,surface,occurred_at:occurredAt};
}
