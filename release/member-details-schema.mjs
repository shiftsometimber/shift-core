...
async function main(){
 ...
 if(!exists){execFileSync(process.execPath,['scripts/catalogue-publication-client.mjs','--verify-main'],{stdio:'inherit'});const result=cli(['d1','execute','DB','--remote','--config','wrangler.jsonc','--json','--file','member-experience/member-details.sql']);assert(result.length&&result.every(x=>x.success),'Additive schema statement failed');}
