...
async function saveDailyAdjustment(request,env){
  ...
  const body=await readClone(request),scenario=String(body.scenario||'').slice(0,30),allowed=['working_late','low_energy','plans_changed'];
  ...
  const date=localDate(request),value={key:scenario,label:{working_late:'Working late',low_energy:'Low energy',plans_changed:'Plans changed'}[scenario],changed_at:new Date().toISOString()};
...
export function composeDailyOutput({date,hour,grubPlan,grubStartsOn,fitPlan,hydrationMl,mode,minutesCap,dayChange,reasons,completedToday,mealOverride=null,rejectedMealIds=[]}){
  ...
  const next=hour<16?{kind:'meal',eyebrow:'NEXT',title:mealOutput.name,detail:mealOutput.ready?mealMeta(mealOutput):'Set your tastes once; Shift will serve the meal here.',cta:mealOutput.ready?'Open recipe':'Build my Grub',href:mealOutput.href}:{kind:'movement',eyebrow:'NEXT',title:workout.title,detail:workout.ready?`${workout.minutes} minutes${workout.first_exercise?` · starts with ${workout.first_exercise}`:''}`:'Set time, place and kit once; Shift will serve the session here.',cta:workout.ready?'Start session':'Build my Fit',href:workout.href};
  return{status:mealOutput.ready&&workout.ready?'ready':'needs_setup',headline:adjusted?'Day rebuilt.':'Today, handled.',subhead:adjusted?changeCopy:'Your real Grub and Fit plans are joined here. Shift serves the useful next action without making you rebuild the day.',adjustment:dayChange,meal:mealOutput,workout,hydration:{logged_ml:hydrationMl,next_ml:250,copy:hydrationMl?`${hydrationMl} ml logged · add 250 ml when useful`:'Start with 250 ml; no perfect target chasing'},next,why:[...reasons.slice(0,2),mealOutput.ready?'Meal selected from your retained Grub plan':'Grub needs setting once',workout.ready?'Movement selected from your retained Fit plan':'Fit needs setting once'],completed_today:completedToday};
}
