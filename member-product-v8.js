import {connectedDay} from './member-experience/journey-context.mjs';
...
export function composeDailyOutput({date,hour,grubPlan,grubStartsOn,fitPlan,hydrationMl,mode,minutesCap,dayChange,reasons,completedToday,mealAccepted=false,recoveryDone=false,mealOverride=null,rejectedMealIds=[],learning={memory:[],prediction:null,weekly:null},treatmentSupport=null}){
  ...
  if(session&&savedFitIssues(fitPlan).length)Object.assign(workout,{ready:false,needsReview:true,completed:false,title:'This saved session needs replacing',detail:'Review it in Fit before starting. Your saved plan and activity history are kept.',minutes:null,exercise_count:0,exercises:[],first_exercise:null});
  ...
  else if(workout.needsReview)next={kind:'fit_review',eyebrow:'NEXT',title:workout.title,detail:workout.detail,cta:'Review saved session',href:'/member/fit'};
