...
export function buildDailyLearning({date,choiceHistory=[],feedbackHistory=[],checkinHistory=[],rejectedMeals=[],recent=[],hydrationMl=0}){const weekday=new Date(`${date}T12:00:00Z`).getUTCDay(),sameDayChanges=choiceHistory.filter(row=>row.domain==='day_change'&&new Date(`${row.local_date}T12:00:00Z`).getUTCDay()===weekday),lowEnergy=checkinHistory.filter(row=>row.energy==='empty').length,memory=[],favouriteMealIds=feedbackHistory.filter(row=>row.feedback==='love').map(row=>String(safeJson(row.context_json).meal_id||'')).filter(Boolean);if(rejectedMeals.length)memory.push(`You ruled out ${rejectedMeals.length} meal${rejectedMeals.length===1?'':'s'}. They are no longer suggested.`);if(favouriteMealIds.length)memory.push('You loved a previous meal. It now wins when it fits the moment.');if(feedbackHistory.some(row=>row.feedback==='effort'))memory.push('You said some ideas took too much effort. Easier options now win ties.');if(feedbackHistory.some(row=>row.feedback==='expensive'))memory.push('You flagged cost. Lower-cost choices now stay visible.');const shortSessions=recent.map(row=>safeJson(row.properties_json)).filter(item=>Number(item.minutes)>0&&Number(item.minutes)<=10).length;if(shortSessions>=2)memory.push('You complete ten-minute sessions more consistently. Shift keeps that version ready.');let prediction=null;if(sameDayChanges.length>=2)prediction={key:'busy_weekday',evidence:`Changed ${sameDayChanges.length} times on this weekday`,copy:'This weekday often changes. The quickest dinner and ten-minute session are already in reserve.',prepared:['quickest dinner','ten-minute movement']};else if(lowEnergy>=2)prediction={key:'low_energy_week',evidence:`Low energy logged ${lowEnergy} times this week`,copy:'Low energy has shown up twice this week. Today starts deliberately lighter.',prepared:['lighter movement','simpler food']};const weekSessions=recent.length,adaptations=choiceHistory.filter(row=>row.domain==='day_change').length,weekly={headline:'What worked this week',copy:`${weekSessions?`${weekSessions} movement session${weekSessions===1?'':'s'} logged. `:'No movement logged yet. '}${shortSessions?'Short sessions worked. ':adaptations?'Changing the plan kept the day alive. ':''}${hydrationMl?'Fluids are moving today. ':''}`,next:`Next week: ${shortSessions?'keep the ten-minute version one tap away':adaptations?'prepare the fallback before the day changes':'start with one useful action, not a perfect day'}.`,observation:shortSessions>=2?`${shortSessions} short sessions completed recently.`:adaptations?`${adaptations} changed day${adaptations===1?'':'s'} rebuilt instead of abandoned.`:rejectedMeals.length?'You adapted your food plan instead of abandoning it.':'Shift needs another completed choice before claiming a pattern.'},mealBias=feedbackHistory.some(row=>row.feedback==='effort')?'effort':feedbackHistory.some(row=>row.feedback==='expensive')?'expensive':null,proof={signals_used:{feedback:feedbackHistory.length,day_changes:choiceHistory.filter(row=>row.domain==='day_change').length,check_ins:checkinHistory.length,movement_sessions:recent.length,excluded_meals:rejectedMeals.length},applied_to_today:[mealBias==='effort'?'easier food wins':null,mealBias==='expensive'?'lower-cost food wins':null,favouriteMealIds.length?'favourites win when suitable':null,rejectedMeals.length?'rejected meals removed':null,shortSessions>=2?'ten-minute movement kept ready':null].filter(Boolean)};return{memory:memory.slice(0,3),prediction,weekly,mealBias,favouriteMealIds,proof}}
