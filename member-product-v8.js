...
export async function memberProductV8Routes(request,env,ctx){
  ...
  if(path==='/v1/shift/daily-feedback'&&request.method==='POST')return saveDailyFeedback(request,env);
...
async function saveDailyMealDecision(request,env){
  ...
  if(!['accept','swap','reject','alternative'].includes(action)||!mealId)return jsonResponse({ok:false,error:'invalid_meal_decision'},400,request);
  ...
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS shift_today_choices (id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,local_date TEXT NOT NULL,domain TEXT NOT NULL,choice_key TEXT NOT NULL,choice_json TEXT NOT NULL,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(user_id,local_date,domain))`).run();
  const [row,overrideRow]=await Promise.all([safeFirst(env.DB,`SELECT plan_json FROM shift_plans WHERE user_id=? AND plan_type='grub' AND status='active' ORDER BY id DESC LIMIT 1`,[auth.user.id]),safeFirst(env.DB,`SELECT choice_json FROM shift_today_choices WHERE user_id=? AND local_date=? AND domain='meal_override'`,[auth.user.id,localDate(request)])]),plan=safeJson(row?.plan_json),meals=(plan.days||[]).flatMap(day=>day?.meals||[]),savedOverride=safeJson(overrideRow?.choice_json),current=meals.find(meal=>String(meal.id||'')===mealId)||(String(savedOverride.id||'')===mealId?savedOverride:null);
  ...
  const wantedType=String(current.type||''),alternatives=meals.filter(meal=>String(meal.id||'')!==mealId&&!blocked.includes(String(meal.id||''))),sameType=alternatives.filter(meal=>String(meal.type||'').toLowerCase()===wantedType.toLowerCase()),pool=sameType.length?sameType:alternatives,option=String(body.option||'quickest');
  const replacement=rankMealAlternatives(pool,current)[option]||rankMealAlternatives(pool,current).quickest;
  ...
  await saveTodayChoice(env.DB,auth.user.id,date,'meal_override',String(replacement.id||''),{...replacement,swapped_from:mealId,reason:action==='reject'?'rejected':option});
  ...
  return jsonResponse({ok:true,action,daily:await fitDailyContext(request,env,auth.user.id),message:action==='reject'?'Understood. Shift will not suggest that meal again.':`${replacement.name} is in. Shift has rebuilt what follows.`},200,request);
}

async function saveDailyFeedback(request,env){
  const auth=await authenticateMember(request,env);if(auth.response)return auth.response;
  const body=await readClone(request),feedback=String(body.feedback||''),target=String(body.target||'today').slice(0,80),allowed=['love','not_again','effort','expensive','wrong_today'];
  if(!allowed.includes(feedback))return jsonResponse({ok:false,error:'invalid_daily_feedback'},400,request);
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS shift_daily_feedback (id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,local_date TEXT NOT NULL,target TEXT NOT NULL,feedback TEXT NOT NULL,context_json TEXT NOT NULL DEFAULT '{}',created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`).run();
  const date=localDate(request),context={meal_id:String(body.meal_id||'').slice(0,120),adjustment:String(body.adjustment||'').slice(0,40)};
  await env.DB.prepare(`INSERT INTO shift_daily_feedback(user_id,local_date,target,feedback,context_json) VALUES(?,?,?,?,?)`).bind(auth.user.id,date,target,feedback,JSON.stringify(context)).run();
  if(feedback==='not_again'&&context.meal_id)await env.DB.prepare(`INSERT INTO shift_meal_preferences(user_id,meal_id,preference) VALUES(?,?,'never') ON CONFLICT(user_id,meal_id) DO UPDATE SET preference='never',updated_at=CURRENT_TIMESTAMP`).bind(auth.user.id,context.meal_id).run();
  await recordProductEvent(env,{userId:auth.user.id,eventName:'daily_recommendation_feedback',surface:'daily_shift_today',source:'member',properties:{date,target,feedback,...context}});
  const messages={love:'Good. Shift will favour choices like this.',not_again:'Understood. That is out of future suggestions.',effort:'Noted. Shift will favour lower-effort options.',expensive:'Noted. Shift will favour cheaper choices.',wrong_today:'Fair. Shift will treat this as today—not a permanent dislike.'};
  return jsonResponse({ok:true,message:messages[feedback],daily:await fitDailyContext(request,env,auth.user.id)},200,request);
}
...
export async function fitDailyContext(request,env,userId){
  ...
  const [checkin,previous,progress,choices,hydration,recent,treatment,grubPlanRow,fitPlanRow,rejectedMeals,choiceHistory,feedbackHistory,checkinHistory]=await Promise.all([
    ...
    safeFirst(env.DB,`SELECT medicine,dose,next_dose_on,next_delivery_on,status FROM shift_treatment_context WHERE user_id=?`,[userId]),
    ...
    safeAll(env.DB,`SELECT local_date,domain,choice_key,choice_json FROM shift_today_choices WHERE user_id=? AND local_date<? ORDER BY local_date DESC LIMIT 80`,[userId,date]),
    safeAll(env.DB,`SELECT local_date,target,feedback,context_json FROM shift_daily_feedback WHERE user_id=? ORDER BY id DESC LIMIT 40`,[userId]),
    safeAll(env.DB,`SELECT local_date,mood,guts,energy FROM shift_today_checkins WHERE user_id=? AND local_date>=date(?,'-7 day') ORDER BY local_date DESC`,[userId,date])
  ]);
  ...
  const learning=buildDailyLearning({date,choiceHistory,feedbackHistory,checkinHistory,rejectedMeals,recent,hydrationMl}),treatmentSupport=buildTreatmentSupport({date,treatment,checkin,dayChange,hydrationMl,protein});
  const baseDailyOutput=composeDailyOutput({date,hour,grubPlan,grubStartsOn:grubPlanRow?.starts_on,fitPlan,hydrationMl,mode,minutesCap,dayChange,reasons,completedToday,mealAccepted:Boolean(saved.grub?.key),recoveryDone,mealOverride:saved.meal_override,rejectedMealIds:rejectedMeals.map(item=>String(item.meal_id)),learning,treatmentSupport}),dailyOutput=applyConfirmedRebuildToDailyOutput(baseDailyOutput,saved.ai_rebuild);
  return{contract:'shift-fit-daily-context/v3',date,mode,intensity,minutes_cap:minutesCap,headline,reasons,checkin:checkin||null,confirmed_rebuild:saved.ai_rebuild||null,recent:{sleep_hours:sleep||null,protein_g:protein||null,hydration_ml:hydrationMl,completed_today:completedToday,completed_sessions_7:activity.sessions_7},activity,progression,today_plan:todayPlan,daily_output:dailyOutput,weekly_guide:{moderate_minutes:150,strength_days:2,source:'NHS physical activity guidelines for adults aged 19 to 64',member_minutes_logged:activity.minutes_7,member_full_sessions_logged:activity.full_sessions_7,note:'This only counts sessions logged in Shift; walking, sport and other activity may also count.'},connections:{food_choice:foodChoice,move_choice:saved.move?.name||null,treatment:treatment?.status==='active'?{medicine:treatment.medicine,dose:treatment.dose,next_dose_on:treatment.next_dose_on}:null},after_session:after,rule:'Current symptoms and safety override progression. Food, fluids, recovery and recent completion inform today’s dose.'};
}

export function applyConfirmedRebuildToDailyOutput(daily,rebuild){
  const meal=rebuild?.selected_meals?.[0],movements=Array.isArray(rebuild?.selected_movements)?rebuild.selected_movements:[],movement=movements[0],plan=rebuild?.plan;if(!plan||!meal||!movement)return daily;
  const canonicalMeal={...(daily.meal||{}),ready:true,accepted:true,id:meal.id,name:meal.title,minutes:Number(meal.minutes||0)||null,href:'/member/grub',alternatives:[]},movementMinutes=movements.reduce((total,item)=>total+Math.max(0,Number(item.minutes)||0),0),canonicalWorkout={...(daily.workout||{}),ready:true,completed:false,title:movement.title,minutes:movementMinutes||Number(movement.minutes)||null,exercise_count:movements.length,exercises:movements.map(item=>item.title),first_exercise:movement.title,href:'/member/fit'},why=[plan.why,...(plan.now||[]),...(plan.next||[])].filter(Boolean);
  return{...daily,status:'ready',headline:plan.headline||'Day rebuilt.',subhead:plan.why||'The confirmed rebuild is now today’s plan.',adjustment:rebuild.route||daily.adjustment,meal:canonicalMeal,workout:canonicalWorkout,next:{kind:'movement',eyebrow:'DO THIS NEXT',title:canonicalWorkout.title,detail:[canonicalWorkout.minutes&&`${canonicalWorkout.minutes} minutes`,plan.next?.[0]].filter(Boolean).join(' · '),cta:'Start session',href:'/member/fit'},why,life_back:plan.life_back||null,monday_amnesty:plan.amnesty||null,canonical_change:rebuild.change_summary||plan.change_summary||null,audit_reference:rebuild.audit_reference||rebuild.proposal_id||null,confirmed_rebuild:true,canonical_today:true};
}

export function composeDailyOutput({date,hour,grubPlan,grubStartsOn,fitPlan,hydrationMl,mode,minutesCap,dayChange,reasons,completedToday,mealAccepted=false,recoveryDone=false,mealOverride=null,rejectedMealIds=[],learning={memory:[],prediction:null,weekly:null},treatmentSupport=null}){
  ...
  if(!dayChange&&learning.favouriteMealIds?.length){const favourite=allowedMeals.find(item=>learning.favouriteMealIds.includes(String(item.id||''))&&wanted.test(String(item.type||'')));if(favourite)meal=favourite}
  if(!dayChange&&learning.mealBias==='effort'){const easier=allowedMeals.filter(item=>wanted.test(String(item.type||''))).sort((a,b)=>Number(a.minutes||999)-Number(b.minutes||999))[0];if(easier)meal=easier}
  if(!dayChange&&learning.mealBias==='expensive'){const cheaper=allowedMeals.filter(item=>wanted.test(String(item.type||''))).sort((a,b)=>Number(a.estimated_cost||a.cost||999)-Number(b.estimated_cost||b.cost||999)||Number(a.minutes||999)-Number(b.minutes||999))[0];if(cheaper)meal=cheaper}
  ...
  const ranked=rankMealAlternatives(allowedMeals.filter(item=>String(item.id||'')!==String(meal?.id||'')),meal),mealOutput=meal?{ready:true,accepted:Boolean(mealAccepted),id:meal.id||null,type:meal.type||'Next meal',name:meal.name,kcal:Number(meal.kcal||0)||null,protein_g:Number(meal.protein||0)||null,minutes:Number(meal.minutes||0)||null,href:'/member/grub',alternatives:Object.entries(ranked).filter(([,item])=>item).map(([key,item])=>({key,label:alternativeLabel(key),id:item.id,name:item.name,minutes:Number(item.minutes||0)||null,kcal:Number(item.kcal||0)||null,protein_g:Number(item.protein||0)||null,consequence:alternativeConsequence(key,item)}))}:{ready:false,accepted:false,type:'Next meal',name:'Build today’s Grub once',kcal:null,protein_g:null,minutes:null,href:'/member/grub',alternatives:[]};
  ...
  const shifted=next.kind==='complete'?{headline:'Today shifted.',copy:adjusted?'You did not follow the original plan—you made the changed day work. That counts.':'You handled the useful things without chasing a perfect day.',observation:learning.weekly?.observation||'You adapted rather than abandoning the day.'}:null;
  return{status:mealOutput.ready&&workout.ready?'ready':'needs_setup',headline:recoveryDone?'Today, properly sorted.':adjusted?'Day rebuilt.':'Today, handled.',subhead:adjusted?changeCopy:(learning.prediction?.copy||'Your real Grub and Fit plans are joined here. Shift serves the useful next action without making you rebuild the day.'),adjustment:dayChange,meal:mealOutput,workout,hydration:{logged_ml:hydrationMl,next_ml:250,copy:hydrationMl?`${hydrationMl} ml logged · add 250 ml when useful`:'Start with 250 ml; no perfect target chasing'},recovery:{completed:Boolean(recoveryDone)},next,why:[...reasons.slice(0,2),mealOutput.ready?`Chosen because it fits this point in the day, takes ${mealOutput.minutes||'a manageable number of'} minutes and comes from your retained Grub plan.`:'Grub needs setting once',workout.ready?`Movement is capped at ${workout.minutes} minutes using your retained Fit plan and today’s energy, symptoms and timing.`:'Fit needs setting once'],memory:learning.memory||[],prediction:learning.prediction||null,weekly_insight:learning.weekly||null,treatment_support:treatmentSupport,shifted,completed_today:completedToday};
}
...
function rankMealAlternatives(meals,current){const list=(meals||[]).filter(Boolean),by=(fn)=>[...list].sort(fn)[0]||null,contains=(pattern)=>list.find(item=>pattern.test(`${item.type||''} ${item.name||''}`))||null;return{quickest:by((a,b)=>Number(a.minutes||999)-Number(b.minutes||999)),cheapest:by((a,b)=>Number(a.estimated_cost||a.cost||999)-Number(b.estimated_cost||b.cost||999)||Number(a.minutes||999)-Number(b.minutes||999)),protein:by((a,b)=>Number(b.protein||b.protein_g||0)-Number(a.protein||a.protein_g||0)),family:contains(/family|tray|pasta|chilli|curry|jacket/i)||by((a,b)=>Number(a.minutes||999)-Number(b.minutes||999)),fakeaway:contains(/fakeaway|tikka|shawarma|salt and pepper|burger|kebab/i)||null,takeaway:{id:'takeaway-simple',type:current?.type||'Dinner',name:'Takeaway, kept simple',minutes:0,kcal:null,protein:null,takeaway:true}}}
function alternativeLabel(key){return{quickest:'Quickest',cheapest:'Cheapest',protein:'Highest protein',family:'Family-friendly',fakeaway:'Fakeaway',takeaway:'Takeaway instead'}[key]||key}
function alternativeConsequence(key,item){return{quickest:`Saves time; movement stays short at ten minutes.`,cheapest:'Keeps the meal lower-cost; the rest of today stays unchanged.',protein:`Puts protein first${item.protein?` at about ${item.protein}g`:''}; no compensating later.`,family:'One shared meal; portions can differ without cooking twice.',fakeaway:'Keeps the takeaway feel while retaining a planned portion.',takeaway:'Choose a protein-led main, one side, water first—and return to the normal plan tomorrow.'}[key]}
function buildDailyLearning({date,choiceHistory,feedbackHistory,checkinHistory,rejectedMeals,recent,hydrationMl}){const weekday=new Date(`${date}T12:00:00Z`).getUTCDay(),sameDayChanges=choiceHistory.filter(row=>row.domain==='day_change'&&new Date(`${row.local_date}T12:00:00Z`).getUTCDay()===weekday),lowEnergy=checkinHistory.filter(row=>row.energy==='empty').length,memory=[],favouriteMealIds=feedbackHistory.filter(row=>row.feedback==='love').map(row=>String(safeJson(row.context_json).meal_id||'')).filter(Boolean);if(rejectedMeals.length)memory.push(`You have ruled out ${rejectedMeals.length} meal${rejectedMeals.length===1?'':'s'}, so Shift has stopped suggesting them.`);if(favouriteMealIds.length)memory.push('You loved a previous meal, so Shift will favour it when it suits the moment.');if(feedbackHistory.some(row=>row.feedback==='effort'))memory.push('You have marked previous ideas as too much effort, so easier options now win ties.');if(feedbackHistory.some(row=>row.feedback==='expensive'))memory.push('You have flagged cost before, so lower-cost choices are kept visible.');const shortSessions=recent.map(row=>safeJson(row.properties_json)).filter(item=>Number(item.minutes)>0&&Number(item.minutes)<=10).length;if(shortSessions>=2)memory.push('Ten-minute sessions are working more consistently for you, so Shift keeps them available.');let prediction=null;if(sameDayChanges.length>=2)prediction={key:'busy_weekday',copy:`This day of the week often changes. Shift has kept the quickest dinner and a ten-minute session in reserve.`};else if(lowEnergy>=2)prediction={key:'low_energy_week',copy:'You have reported low energy twice this week. Today is deliberately lighter.'};const weekSessions=recent.length,weekly={headline:'Your week, without the spreadsheet.',copy:`${weekSessions?`${weekSessions} movement session${weekSessions===1?'':'s'} logged. `:''}${shortSessions?'Short sessions worked. ':''}${hydrationMl?'Fluids are moving today. ':''}Next week, Shift will keep learning from what you actually complete.`,observation:shortSessions>=2?`${shortSessions} short sessions completed recently.`:rejectedMeals.length?'You adapted your food plan instead of abandoning it.':'You kept the day moving without demanding perfection.'},mealBias=feedbackHistory.some(row=>row.feedback==='effort')?'effort':feedbackHistory.some(row=>row.feedback==='expensive')?'expensive':null;return{memory:memory.slice(0,3),prediction,weekly,mealBias,favouriteMealIds}}
function buildTreatmentSupport({date,treatment,checkin,dayChange,hydrationMl,protein}){if(treatment?.status!=='active')return null;const doseDay=treatment.next_dose_on===date,rough=checkin?.guts==='rough'||['feeling_rough','rough_guts'].includes(dayChange),lowAppetite=checkin?.energy==='empty'||rough,notes=[];if(doseDay)notes.push(`${treatment.medicine} day is recorded today.`);if(lowAppetite)notes.push('Appetite or digestion may be harder today, so food is smaller and protein-led and fluids are spread out.');if(hydrationMl<750)notes.push('Fluids are still low; sip steadily rather than catching up before bed.');if(protein>0&&protein<70)notes.push('Protein is running light, so the next tolerable meal prioritises it.');if(treatment.next_delivery_on)notes.push(`Your recorded reorder point is ${treatment.next_delivery_on}.`);return{headline:lowAppetite?'Treatment-aware support is on.':'Treatment context is included.',notes,escalation:rough?'If symptoms are severe, persistent, worsening, or you cannot keep fluids down, seek clinical advice; use NHS 111 when urgent. Shift does not diagnose or alter treatment.':'Shift supports the day around treatment but does not diagnose, prescribe or change your dose.'}}
