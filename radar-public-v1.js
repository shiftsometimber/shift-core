...
import {readPublishedTickerEdition} from './radar-published-ticker-v1.js';
...
export async function radarPublicRoutes(request,env){
 ...
 if(path==='/v1/radar/ticker'){
  ...
  const published_edition=await readPublishedTickerEdition(env.DB);
  return json({ok:true,current:Boolean(freshness.current&&items.length),status:freshness.status,serving:'governed_published_wire',freshness,message:items.length?null:'No governed published ticker items.',items,published_edition});
 }
