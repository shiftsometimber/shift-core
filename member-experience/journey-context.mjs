import {nextShiftCard} from './life-back/next-shift.mjs';
...
export function reflectionWeek(entries,date){
 const anchor=new Date(date+'T12:00:00Z'),offset=(anchor.getUTCDay()+6)%7;
 anchor.setUTCDate(anchor.getUTCDate()-offset);
 const dates=Array.from({length:7},(_,i)=>new Date(anchor.getTime()+i*86400000).toISOString().slice(0,10));
 const loggedDates=[...new Set(entries.filter(e=>Number.isFinite(Date.parse(e.at))).map(e=>ukDate(new Date(e.at))).filter(day=>dates.includes(day)&&day<=date))].sort();
 return {dates,loggedDates};
}
export async function connectedDay(DB,userId,prefs,date=ukDate()){
 ...
 const weekDates=new Set(reflectionWeek([],date).dates.filter(day=>day<=date));const weekMovement=Object.values(prefs.fitJourney?.entries||{}).filter(e=>weekDates.has(e.recordedOn||(Number.isFinite(Date.parse(e.updatedAt))?ukDate(new Date(e.updatedAt)):'')));
 ...
 return {date,meal,week:reflectionWeek(state.entries,date),weekMeals:prefs.grubV2?.week?.length||0,weekMovement:{done:weekMovement.filter(e=>e.status==='done').length,skipped:weekMovement.filter(e=>e.status==='skipped').length},movement:{done,skipped},lifeBack:{score:s.score,change:s.change,at:last?.at||null,win:state.win,goal:state.goal,entries:state.entries.length},mood:mood?{label:['Tough day','Struggling','OK','Good','Brilliant'][mood.wellbeing_score-1],at:mood.submitted_at}:null,next};
}
