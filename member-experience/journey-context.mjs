...
export async function connectedDay(DB,userId,prefs,date=ukDate()){
 ...
 return {date,meal,week:reflectionWeek(state.entries,date),weekMeals:prefs.grubV2?.week?.length||0,movement:{done,skipped},lifeBack:{score:s.score,change:s.change,at:last?.at||null,win:state.win,goal:state.goal,entries:state.entries.length},mood:mood?{label:['Tough day','Struggling','OK','Good','Brilliant'][mood.wellbeing_score-1],at:mood.submitted_at}:null,next};
}
