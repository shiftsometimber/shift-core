export const fitSessionCues=String.raw`
  function refreshSessionCue(session){
    ...
    timing.textContent=session.dataset.sessionMinutes?'Estimated movement time: '+session.dataset.sessionMinutes+' minutes.'+(session.dataset.requestedMinutes?' Your selected time window is '+session.dataset.requestedMinutes+' minutes.':'')+' Rest as needed; do not rush to fit the clock.':'Follow the repetitions or timing shown for each movement. Take the listed rest.';
