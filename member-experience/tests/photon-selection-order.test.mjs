...
for(const existing of ['', '12 Example Street', 'Different flat'])test('licensed line 2 selection handles existing value: '+existing,async()=>{
 const h=setup('12 Example Street');await h.win.dispatchEvent(new h.E('memberAddressLoaded',{detail:{enabled:true,provider:'ideal-postcodes'}}));h.get('memberPostcode').value='SW1A 1AA';h.get('memberAddress2').value=existing;
 await h.get('memberFindAddress').dispatchEvent(new h.E('click'));const select=h.get('memberAddressSelect');select.value='0';await select.dispatchEvent(new h.E('change',{bubbles:true}));
 if(existing==='Different flat'){assert.equal(h.get('memberAddress1').value,'');assert.equal(h.get('memberAddress2').value,existing);assert.match(h.get('memberAddressHelp').textContent,/different address line 2/);}
 else {assert.equal(h.get('memberAddress1').value,'12 Example Street');assert.equal(h.get('memberAddress2').value,'12 Example Street');}
});
