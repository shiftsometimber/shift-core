 ...
 const panel=`<section id="memberDetailsPanel" aria-labelledby="memberDetailsTitle">
  ...
  <form id="memberDetailsForm">
   <fieldset id="memberDetailsFields" disabled><legend class="md-sr">Your personal and contact details</legend>
    ...
    <div class="md-section"><h3>Home address</h3>
     <label for="memberPostcode">Postcode<input id="memberPostcode" name="postcode" autocomplete="postal-code" maxlength="12" aria-describedby="memberAddressHelp"></label>

    <p class="md-help" id="memberAddressHelp" role="status" aria-live="polite">Enter your full address below.</p>
...
let revision=null,loaded=false,busy=false,pending=null,dirty=false,gpEnabled=false,gpTimer=null,gpSerial=0,gpResults=[];
...
async function load(){if(busy)return;busy=true;controls(true);message('Loading your saved details…');try{const r=await api('/v1/member/details');fill(r);pending=null;controls(false);reload.hidden=true;message('Your saved account details are shown above.');}catch(e){message(e.message);reload.hidden=false;reload.textContent='Retry loading details';controls(true);}finally{busy=false;}}
...
form.addEventListener('submit',async event=>{event.preventDefault();if(!loaded||busy||save.disabled)return;if(!pending)pending={details:payload(),revision,operationId:crypto.randomUUID()};busy=true;controls(true);reload.hidden=true;message('Saving your details…');try{const r=await api('/v1/member/details','PATCH',pending);fill(r);pending=null;controls(false);message('Member details saved.');save.textContent='Save member details';window.dispatchEvent(new Event('memberDetailsSaved'));}catch(e){message(e.message);reload.hidden=false;reload.textContent='Reload saved details';if(e.uncertain){fields.disabled=true;save.disabled=false;save.textContent='Retry save';}else{pending=null;controls(false);if(e.status===409||e.status===401)save.disabled=true;}}finally{busy=false;}});
