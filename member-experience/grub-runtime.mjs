...
async function mutate(action,success){
 if(busy||!workspace)return false;busy=true;lock();message('Saving to your account…');
 ...
 try{workspace=await api('workspace',pending.body);pending=null;draw();message(success);return true}
...
if($('#grubStartTrial'))$('#grubStartTrial').onclick=async()=>{const b=$('#grubStartTrial');b.disabled=true;message('Creating a private fictional test account…');try{const r=await fetch('/staging/food-trial',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});if(!r.ok)throw Error('Could not start the test account. Please try again.');await load();await discover()}catch(e){message(e.message,true)}finally{b.disabled=false}};
...
export function upgradeGrubHTML(html,{staging=false}={}){
 ...
 const status='<div class="grub-account"><p id="grubAccountStatus" role="status" aria-live="polite">Loading your saved food…</p><div class="grub-actions"><button id="grubReload">Reload saved food</button><a href="'+(staging?'/staging/sign-in':'/member/dashboard')+'">Sign in</a><button id="grubSignOut">Sign out</button>'+(staging?'<button id="grubStartTrial">Start a private test account</button>':'')+'</div></div>';
