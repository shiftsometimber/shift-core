...
function render(host,r){const list=ideas(r);host.innerHTML=`<p style="grid-column:1/-1" role="status">${esc(r?.message||'')}</p>`+(list.length?list.slice(0,12).map(recipe).join(''):'<div class="grub-empty"><strong>No matching recipe.</strong><span>Try one ingredient or clear the filter.</span></div>')}
...
export function improveGrubClient(original){
 ...
 section('function recipe(m){','function ideas(r){',recipe.toString());
 section('function render(host,r){','function drawChips(){',render.toString());
