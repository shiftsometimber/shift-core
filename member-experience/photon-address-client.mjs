...
for(const c of configs){
 ...
 if(!form||!query||query.dataset.bound)continue;query.dataset.bound='true';let serial=0,abort=null,results=[],enabled=false,selecting=false;
 ...
 button.addEventListener('click',async()=>{
  ...
  const n=serial,before=fingerprint(),controller=new AbortController();abort=controller;const timer=setTimeout(()=>controller.abort(),8000);button.disabled=true;help.textContent='Searching open address data…';
  try{
   const r=await fetch('/v1/member/details/address-search',{method:'POST',credentials:'include',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify({postcode,query:terms}),signal:controller.signal});
   ...
   help.textContent=results.length?'Choose a suggestion, then check the house or flat and every address field. This is not postal verification.':'No complete matching address was found. Add a building or street and search again, or enter the address manually.';
 ...
 select.addEventListener('change',()=>{
  ...
  // Values are copied only after deliberate selection. Flat/address-line-2 and
  // recipient fields are never replaced; missing source fields never erase edits.
  const changes=[[c.address1,a.address1],[c.town,a.town],[c.county,a.county],[c.postcode,a.postcode]];selecting=true;
