...
export function memberNavigation(work=false){
 const links=[['/member/dashboard#today','Today'],['/member/dashboard#journey','Journey'],['/member/grub','Grub'],['/member/fit','Fit'],['/member/check-in','Check-in'],['/member/dashboard#visualise','Progress']];
 return '<span class="member-nav-label">MY TIMBER</span><div class="member-nav-tools">'+links.map(([href,label])=>'<a href="'+href+'">'+label+'</a>').join('')+'</div><details class="member-nav-more"><summary>Plans &amp; records</summary><div><a href="/member/life-back">Life Back · goals &amp; wins</a><a class="mp-tab" data-panel="plans" href="/member/dashboard#plans">My Plans</a><a href="/member/saved">Saved &amp; records</a><a href="/member/orders">My Orders</a><a href="/member/settings#memberDetailsPanel">Member details</a><a href="/member/settings">Settings &amp; privacy</a><a href="/member/ask-timber">Ask Timber</a>'+(work?'<a href="/member/work">My workplace programme</a>':'')+'<form class="member-nav-logout-form" method="post" action="/v1/auth/logout"><button type="submit" class="member-nav-logout" data-member-logout>Log out</button></form></div></details>';
}
...
html body[data-member-chrome="v1"][data-member-page] nav.sst-member-tabs .member-nav-more .member-nav-logout-form{margin:0;padding:0}
