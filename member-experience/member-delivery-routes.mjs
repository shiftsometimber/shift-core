...
function view(s,env){return {ok:true,delivery:s.delivery,home:s.home,effectiveAddress:s.delivery.useHome?s.home:Object.fromEntries(Object.keys(fields).map(k=>[k,s.delivery[k]])),configured:Boolean(s.contact.delivery),revision:s.revision,addressLookupConfigured:addressLookupEnabled(env),addressLookupProvider:addressLookupProvider(env),orderAddressChanged:false};}
