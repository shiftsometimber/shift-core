...
for(const p of pins){
  const data=p.source==='worker'?readFileSync(resolve('frontend/member',p.path)):process.env.SHIFT_WORK_PAGES_ROOT?readFileSync(resolve(process.env.SHIFT_WORK_PAGES_ROOT,p.path)):Buffer.from(await(await fetch('https://0da69833.projectshift.pages.dev/'+p.path)).arrayBuffer());
