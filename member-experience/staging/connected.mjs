...
export async function connectedMemberRoutes(request,env){
 ...
 const name=canonical?path.slice(8):path.slice(prefix.length),jsHeaders={...headers,'Content-Type':'text/javascript'};
 if(name==='life-back'){
 ...
 html=html.replace(/(href=")(\/[^"?]+\.css)([^\"]*")/g,(_,a,b,c)=>a+(currentToolAssets.includes(b.slice(1))?b:source+b.slice(1))+c);
