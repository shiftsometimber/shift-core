...
const checkerHTML='<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>My Timber review</title><script defer src="/staging/member/checker.mjs"></script><style>body{margin:20px;background:#e7e3da;color:#11140f;font:16px/1.5 Arial}label{display:inline-block;margin:0 16px 12px 0}select{font:inherit;padding:10px;min-height:44px}iframe{display:block;height:850px;border:1px solid #707762;background:#e7e3da}</style></head><body><h1>My Timber review</h1><p>Existing member tools, revised presentation. Fictional data; saves disabled.</p><label>Screen <select id="screen">'+Object.keys(scripts).map(x=>'<option value="'+x+'">'+x+'</option>').join('')+'</select></label><label>Viewport width <select id="width"><option>320</option><option>375</option><option selected>390</option><option>768</option><option>1024</option><option>1280</option></select></label><p role="status" id="metrics">Measuring…</p><iframe id="layout-frame" title="Fictional My Timber screen" src="/staging/member/dashboard#journey" width="390"></iframe></body></html>';
const checkerJS=String.raw`const frame=document.querySelector('#layout-frame'),metrics=document.querySelector('#metrics');let observer;function measure(){const doc=frame.contentDocument;if(!doc)return;const area=doc.documentElement.clientWidth,content=Math.max(doc.documentElement.scrollWidth,doc.body.scrollWidth);metrics.textContent=frame.width+'px frame · '+area+'px content area · '+content+'px content width · '+(content<=area?'No horizontal overflow':'Horizontal overflow detected')}frame.addEventListener('load',()=>{observer?.disconnect();observer=new MutationObserver(measure);observer.observe(frame.contentDocument.body,{childList:true,subtree:true});measure()});document.querySelector('#width').addEventListener('change',e=>{frame.width=e.target.value;requestAnimationFrame(measure)});document.querySelector('#screen').addEventListener('change',e=>{metrics.textContent='Measuring…';frame.src='/staging/member/'+e.target.value+(e.target.value==='dashboard'?'#journey':'')});`;
