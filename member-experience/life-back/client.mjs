import {areas,goalChoices,summary,score,lifeBackState,dailyTrend} from '/assets/member-experience/life-back/model.mjs';
...
$('#checkinForm').addEventListener('submit',async e=>{e.preventDefault();if(busy||!loaded)return;const ratings=readRatings();if(score(ratings)===null||Object.values(ratings).some(v=>!Number.isInteger(v))){$('#checkinError').textContent='Rate all six areas from 0 to 100, or keep this as a draft.';return}try{await save({action:'checkin',goalId:state.goalId,ratings,win:$('#winInput').value});draft=null;render();$('#checkinDialog').close();status('Check-in saved to your account. Today and your graph are up to date.')}catch(err){$('#checkinError').textContent=err.message}});
$('#saveDraft').addEventListener('click',()=>{draft={ratings:readRatings(),win:$('#winInput').value};render();$('#checkinDialog').close();status('Draft kept on this page. Your saved score and history have not changed.')});
...
function lock(){document.querySelectorAll('[data-dialog],form button,form input,form textarea').forEach(el=>el.disabled=busy||!loaded)}
