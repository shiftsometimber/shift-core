...
function render(){
 ...
 const first=$('#firstRecordPrompt');first.replaceChildren();if(!last){const hint=document.createElement('span');hint.textContent='Example: “I joined a short walk.” Save your own small win when you are ready. ';const button=document.createElement('button');button.type='button';button.dataset.dialog=hasPersonalGoal(state.goal)?'checkinDialog':'goalDialog';button.textContent=hasPersonalGoal(state.goal)?'Add your first reflection':'Choose your first goal';first.append(hint,button)}
 ...
 $('#trendFeedback').textContent=trend.length<2?(trend.length?'No trend yet. Add another reflection on a later day to start comparing.':'No trend yet. Save your first reflection to set a baseline.'):(()=>{const diff=Math.round(trend.at(-1).value-trend[0].value);return 'Your daily average is '+Math.abs(diff)+' points '+(diff>=0?'higher':'lower')+' than '+trend[0].label+'. This describes the moments you logged; it does not explain what caused the change.'})();
...
async function load(){busy=true;loaded=false;lock();status('Loading your private history…');$('#historyLoading').hidden=false;$('#main').setAttribute('aria-busy','true');$('#accountError').hidden=true;try{const r=await request('/v1/life-back');state=lifeBackState(r.progress);legacyEntries=Array.isArray(r.legacyEntries)?r.legacyEntries:[];loaded=true;render();measurements(r.journey||{});$('#journeyView').hidden=false;$('#accountError').hidden=true;status('');if(location.hash==='#check-in')open('checkinDialog');}catch(e){$('#journeyView').hidden=true;$('#accountError').hidden=false;$('#accountErrorText').textContent=e.message;$('#accountError a').hidden=e.status!==401;status('')}finally{$('#historyLoading').hidden=true;$('#main').setAttribute('aria-busy','false');busy=false;lock()}}
$('#reloadHistory').addEventListener('click',load);$('#retryHistory').addEventListener('click',load);load();
