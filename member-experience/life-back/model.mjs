...
export function lifeBackState(progress){
 const p=progress||{goalId:'personal-start',goal:'Your personal goal',entries:[],revision:0};
 const entries=(p.entries||[]).filter(e=>score(e.ratings)!==null).sort((a,b)=>a.at.localeCompare(b.at)),current=entries.filter(e=>e.goalId===p.goalId),blank=Object.fromEntries(areas.map(a=>[a.id,null]));
 return {...p,entries,ratings:current.at(-1)?.ratings||blank,previous:current.at(-2)?.ratings||blank,win:[...entries].reverse().find(e=>e.win)?.win||'Your next small win belongs here.'};
}
