...
async function boundedJSON(url,init={},transport=fetch){
 const r=await transport(url,{...init,redirect:'manual',signal:AbortSignal.timeout(6000)});
 if(!r.ok||r.status>=300)throw Error('provider_unavailable');
 const text=await r.text();if(text.length>200000)throw Error('provider_too_large');return JSON.parse(text);
}
