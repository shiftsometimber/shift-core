import {sourceReviewGuardSql,sourceReviewGuardBindings} from './radar-source-review-v1.js';
...
export async function upsertKnowledge(env,item,reviewEvent=null){await ensureGraphSchema(env.DB);const v=validate(item);if(!v.ok)return{accepted:false,reason:v.reason};const guard=reviewEvent?` WHERE EXISTS (SELECT 1 FROM radar_events e WHERE e.id=? AND e.status IN ('approved','publish_failed','published') AND ${sourceReviewGuardSql('e')})`:'',guardBindings=reviewEvent?[reviewEvent.id,...sourceReviewGuardBindings(reviewEvent)]:[],data=reviewEvent?{...item.data,source_radar_event_id:reviewEvent.id,source_radar_review_generation:Number(reviewEvent.source_review_generation||0)}:item.data||{};const written=await env.DB.prepare(`INSERT INTO shift_knowledge_nodes(id,node_type,domain,label,data_json,status,updated_at) SELECT ?,?,?,?,?,'active',CURRENT_TIMESTAMP${guard} ON CONFLICT(id) DO UPDATE SET node_type=excluded.node_type,domain=excluded.domain,label=excluded.label,data_json=excluded.data_json,status='active',updated_at=CURRENT_TIMESTAMP`).bind(String(item.id),String(item.type),String(item.domain||'general'),String(item.label),JSON.stringify(data),...guardBindings).run();if(written.meta?.changes===0)return{accepted:false,reason:'source_changed_review_required'};for(const s of item.sources||[])await env.DB.prepare(`INSERT INTO shift_knowledge_sources(node_id,source_type,source_ref,authority,verified_at,expires_at,provenance_json) SELECT ?,?,?,?,?,?,?${guard}`).bind(String(item.id),String(s.type||'source'),String(s.ref||s.url||''),Number(s.authority||50),s.verified_at||null,s.expires_at||null,JSON.stringify(s.provenance||{}),...guardBindings).run();for(const e of item.edges||[])if(e.to&&e.type)await env.DB.prepare(`INSERT INTO shift_knowledge_edges(from_id,edge_type,to_id,weight,data_json) SELECT ?,?,?,?,?${guard} ON CONFLICT(from_id,edge_type,to_id) DO UPDATE SET weight=excluded.weight,data_json=excluded.data_json`).bind(String(item.id),String(e.type),String(e.to),Number(e.weight||1),JSON.stringify(e.data||{}),...guardBindings).run();return{accepted:true,id:item.id}}
