import {runLiveArticleEmails} from './radar-live-email-v1.js';
...
export async function runRadarScheduledScan(env){
  const publicationEmails=await runLiveArticleEmails(env).catch(error=>({sent:0,error:String(error?.message||error)}));
  ...
  return{scan,reviewQueue,editorialCadence,freshness,publicationEmails};
}
