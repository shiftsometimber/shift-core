...
green('10. Failures remain visible or fail into a safe fallback',/That did not save\. Try once more\./.test(today)&&/Try again/.test(today)&&/fallbackFitPlan/.test(fit)&&/No generic workout has replaced your personalised session\./.test(fit)&&/optional short walking break is available while you retry Fit\./.test(fit)&&/Could not build your week/.test(grub));
