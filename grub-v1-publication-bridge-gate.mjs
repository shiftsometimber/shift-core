...
if(digests.length!==8||new Set(launch.recipeIds||[]).size!==783)throw new Error('V1 launch cohort must be exactly 8 immutable decisions / 783 recipes');

const decisionFile=path.join(root,'all-pass.json');
fs.writeFileSync(decisionFile,JSON.stringify({
  proof:'M11_SECOND_PERSON_DECISIONS',
  source_summary:{requiredRecipes:783,requiredTemplateDecisions:8},
  decisions:digests.map(template_digest=>({template_digest,decision:'PASS'}))
},null,2));
...
if(passSummary.decisionCount!==8||passSummary.approvedRecipes!==783||passSummary.heldRecipes!==0||passSummary.publicationReady!==true)throw new Error(`all-PASS publication bridge mismatch: ${JSON.stringify(passSummary)}`);
if(!Array.isArray(payload.items)||payload.items.length!==783)throw new Error('all-PASS publication payload must contain exactly 783 reviewed+validated records');
...
fs.writeFileSync(holdFile,JSON.stringify({
  proof:'M11_SECOND_PERSON_DECISIONS',
  source_summary:{requiredRecipes:783,requiredTemplateDecisions:8},
  decisions:digests.map(template_digest=>template_digest===fixedDigest?{template_digest,decision:'FIX',note:'commissioning hold proof'}:{template_digest,decision:'PASS'})
},null,2));
...
const report={
  proof:'M11_V1_DECISION_TO_PUBLICATION_BRIDGE_V1',
  immutableDecisions:8,
  boundRecipes:783,
  allPass:{approved:783,held:0,publicationReady:true,payloadRecords:payload.items.length},
  nonPass:{decision:'FIX',held:holdSummary.heldRecipes,publicationReady:false,partialPayloadEmitted:false},
  criterion:'A complete immutable eight-decision PASS set yields exactly 783 reviewed+validated publication records; any FIX/REJECT fails the V1 launch bridge closed with no partial payload.'
};
fs.writeFileSync(path.join(evidenceDir,'report.json'),JSON.stringify(report,null,2));
console.log(JSON.stringify(report,null,2));
console.log('PASS M11 V1 decision-to-publication bridge: technically ready for the finite eight-decision human review; publication remains held until genuine decisions are returned.');
