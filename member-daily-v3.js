...
export function resolveTreatmentSetup(body={}){const medicineKey=clean(body.medicineKey,30),doseKey=clean(body.doseKey,30),durationKey=clean(body.durationKey,30);if(['not-taking','add-later'].includes(medicineKey))return{medicine:medicineKey,route:'none',dose:'n/a',week:null,status:medicineKey==='not-taking'?'not_taking':'add_later'};const medicines={mounjaro:{name:'Mounjaro',route:'jab',doses:['2.5mg','5mg','7.5mg','10mg','12.5mg','15mg']},wegovy:{name:'Wegovy',route:'jab',doses:['0.25mg','0.5mg','1mg','1.7mg','2.4mg']},tablet:{name:'A tablet',route:'tablet',doses:['60mg','120mg']}}[medicineKey],weeks={started:1,weeks:3,months:8,longer:16};if(!medicines||!medicines.doses.includes(doseKey)||!weeks[durationKey])return null;return{medicine:medicines.name,route:medicines.route,dose:doseKey,week:weeks[durationKey],status:'active'}}
