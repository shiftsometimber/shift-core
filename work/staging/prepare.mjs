...
import '../../member-experience/staging/grub-catalogue.mjs';
...
for(const p of pins){const data=process.env.SHIFT_WORK_PAGES_ROOT?readFileSync(resolve(process.env.SHIFT_WORK_PAGES_ROOT,p.path)):Buffer.from(await (await fetch('https://0da69833.projectshift.pages.dev/'+p.path)).arrayBuffer());if(createHash('sha256').update(data).digest('hex')!==p.sha256)throw Error('Pinned asset mismatch: '+p.path);const dest=dir+'/assets/'+p.path;mkdirSync(dirname(dest),{recursive:true});writeFileSync(dest,data)}
...
const closedEnd=new Date(+start-86400000),closed={id:closedId,revision:0,config:{...config,name:'Fictional closed reporting cohort',start:new Date(+closedEnd-84*86400000).toISOString().slice(0,10),end:closedEnd.toISOString().slice(0,10)},status:'active',members:Array.from({length:35},(_,i)=>({userId:900000+i,joinedAt:'2026-01-01T00:00:00.000Z',noticeVersion:'work-pilot-2026-09-12-v1',completedWeeks:i<15?Array.from({length:12},(_,j)=>j+1):i<25?[1]:[]})),invites:[],audit:[]};
...
const catalogue=await loadGovernedGrubCatalogue({prepare(sql){return{bind(...args){return{async all(){return{results:catalogueDB.prepare(sql).all(...args)}}}}}}});
if(catalogue.authority.incomplete||catalogue.authority.accepted!==798)throw Error('Staging accepted recipe catalogue failed: '+JSON.stringify(catalogue.authority));
catalogueDB.close();
