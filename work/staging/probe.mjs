...
checks.push(...await probeLifeBack({call,member:employee,other,relogin:()=>login(fixture.ids[0]),patch:(path,body,cookie)=>fetch(origin+path,{method:'PATCH',headers:{Origin:origin,'Content-Type':'application/json',Cookie:cookie},body:JSON.stringify(body)}),method:(path,method,body,cookie)=>fetch(origin+path,{method,headers:{Origin:origin,'Content-Type':'application/json',Cookie:cookie},body:body===undefined?undefined:JSON.stringify(body)})}));
