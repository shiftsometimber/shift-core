...
const report={at:new Date().toISOString(),commit:process.env.GITHUB_SHA,cases:[],failures:[],limits:['Fictional isolated accounts only','Session latency and503 responses for session, Life Back, Grub load and Fit build are deliberately injected for recovery checks; successful session and day adjustment use real APIs','No password reset email or real member writes']};
...
try{
 for(const [index,[name,viewport]]of Object.entries({desktop:{width:1440,height:1000},mobile390:{width:390,height:844}}).entries()){
  ...
  try{
   ...
   await page.locator('#memberDayGuide li').first().waitFor();assert.equal(await page.locator('#memberDayGuide li').count(),4);assert((await page.locator('#memberDayGuide').innerText()).includes('Your week and latest feedback'));await shot(page,'today-first-day-'+name);
   await page.locator('.member-nav-tools a').filter({hasText:/^Progress$/}).click();await page.locator('.member-progress-map').waitFor({state:'visible'});await shot(page,'progress-door-'+name);row.checks.push('Today shows four ordered steps and honest saved state; visible Progress link opens the measurements, Life Back and photos map');
   await page.goto(origin+'/member/grub');await page.waitForFunction(()=>document.querySelector('#grubAccountStatus')?.textContent.includes('up to date'));
   await page.locator('#grubSearch').fill('chicken');await page.locator('#grubSearchGo').click();const foodToday=page.locator('#grubDiscoverResults [data-food-today]').first();await foodToday.waitFor();await foodToday.click();await page.locator('#grubDiscoverResults [data-food-receipt]').first().waitFor({state:'visible'});assert(await page.locator('#grubActionReceipt').getByRole('link',{name:'See your meal on Today →',exact:true}).isVisible());await shot(page,'grub-receipt-'+name);row.checks.push('Real food choice gives persistent confirmation and direct Today destination');
   await page.goto(origin+'/member/fit');await page.locator('#fitGenerate').waitFor();await page.locator('#fitDays').selectOption('1');await page.locator('#fitMinutes').selectOption('10');const built=page.waitForResponse(r=>new URL(r.url()).pathname==='/v1/fit/plan'&&r.request().method()==='POST');await page.locator('#fitGenerate').click();assert.equal((await built).status(),200);await page.locator('.sf-current-step').first().waitFor();
   const titles=await page.locator('.sf-exercise h4').allTextContents();assert(titles.length>0);assert(titles.every(t=>!/Cool-Down|Warm-Up/i.test(t)),'Main-session build must not select phase variants');assert(titles.every(t=>!/(?:Advanced|Standard)$/.test(t)),'Default beginner build must not use higher difficulty variants');const exercise=page.locator('.sf-exercise:not([data-completion="done"]):not([data-completion="skipped"])').first();if(await exercise.count()){await exercise.locator('[data-sf-complete="done"]').click();await page.waitForFunction(()=>document.querySelector('.sf-exercise[data-completion="done"]'));}await page.locator('.sf-current-step').first().scrollIntoViewIfNeeded();await shot(page,'fit-current-movement-'+name);row.checks.push('Real short Fit build excludes phase variants, shows movement order and saves Done before advancing the cue');
   let releaseHistory;const historyHold=new Promise(resolve=>releaseHistory=resolve);await page.route('**/v1/life-back',async route=>{await historyHold;await route.continue()});await page.goto(origin+'/member/life-back',{waitUntil:'domcontentloaded'});assert(await page.locator('#historyLoading').isVisible());assert(!(await page.locator('#journeyView').isVisible()));await shot(page,'life-back-loading-'+name);releaseHistory();await page.locator('#journeyView').waitFor({state:'visible'});await page.unroute('**/v1/life-back');
   await page.route('**/v1/life-back',route=>route.fulfill({status:503,contentType:'application/json',body:'{"error":"Fictional history interruption"}'}));await page.reload();await page.locator('#accountError').waitFor({state:'visible'});assert(!(await page.locator('#accountError a').isVisible()));await page.unroute('**/v1/life-back');await page.locator('#retryHistory').click();await page.locator('#journeyView').waitFor({state:'visible'});row.checks.push('Delayed Life Back shows a real loading layout;503 shows retry without suggesting sign-out; retry restores actual history');
   const blank=await browser.newContext({viewport,reducedMotion:'reduce'});try{
    await login(blank,fixture.browserIds[index*2+1]);const first=await blank.newPage();await first.goto(origin+'/member/dashboard#journey');await first.getByRole('button',{name:'Add your starting point',exact:true}).waitFor({state:'visible'});assert.equal(await first.locator('.mj-stat-grid').count(),0);await shot(first,'journey-first-use-'+name);await first.getByRole('button',{name:'Add your starting point',exact:true}).click();assert(await first.locator('[data-mj-form]').isVisible());assert(await first.locator('[name=why]').isVisible());assert(!(await first.locator('[name=heightCm]').isVisible()));assert.equal(await first.locator('[name=life_energy]').inputValue(),'');await shot(first,'journey-guided-form-'+name);row.checks.push('Empty Journey shows honest starting-point CTA; opens existing form without a write');
  ...
  }catch(error){row.error=String(error.message).replaceAll(fixture.password,'[redacted]');report.failures.push({name,error:row.error});console.error(name+': '+row.error);}
