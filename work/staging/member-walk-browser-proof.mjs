...
try{
 for(const [index,[name,viewport]]of Object.entries({desktop:{width:1440,height:1000},mobile390:{width:390,height:844}}).entries()){
  ...
  try{
   ...
   await page.locator('#grubSearch').fill('chicken');await page.locator('#grubSearchGo').click();const foodToday=page.locator('#grubDiscoverResults [data-food-today]').first();await foodToday.waitFor();await foodToday.click();await page.getByRole('region',{name:'Saved food confirmation'}).waitFor({state:'visible'});assert(await page.locator('#grubActionReceipt').getByRole('link',{name:'See your meal on Today →',exact:true}).isVisible());await shot(page,'grub-receipt-'+name);row.checks.push('Real food choice gives persistent confirmation and direct Today destination');
   ...
   const titles=await page.locator('.sf-exercise h4').allTextContents();assert(titles.length>0);assert(titles.every(t=>!/Cool-Down|Warm-Up/i.test(t)),'Main-session build must not select phase variants');const exercise=page.locator('.sf-exercise:not([data-completion="done"]):not([data-completion="skipped"])').first();if(await exercise.count()){await exercise.locator('[data-sf-complete="done"]').click();await page.waitForFunction(()=>document.querySelector('.sf-exercise[data-completion="done"]'));}await shot(page,'fit-current-movement-'+name);row.checks.push('Real short Fit build excludes phase variants, shows movement order and saves Done before advancing the cue');
