...
try{
 for(const [index,[name,viewport]]of Object.entries({desktop:{width:1440,height:1000},mobile390:{width:390,height:844}}).entries()){
  ...
  try{
   ...
   await page.goto(origin+'/member-login');await page.waitForURL(origin+'/member/dashboard');await page.waitForFunction(()=>document.body.dataset.memberSession==='ready');row.checks.push('Retained session on member-login lands on the canonical dashboard');
