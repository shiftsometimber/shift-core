import {addMemberChrome,memberNavigation} from '../member-experience/chrome.mjs';
...
export function workHTML(mode){
 ...
 const html=programmeHTML.replace(/<title>.*?<\/title>/,`<title>${title}</title>`).replace('/assets/programme-v1/programme.css','/assets/work/work.css').replace('/assets/programme-v1/programme.mjs','/assets/work/work.mjs').replace(/<main\b.*?<\/main>/s,main);
 return mode==='member'?addMemberChrome(html.replace('<main class="work">','<nav class="sst-member-tabs" aria-label="My Timber">'+memberNavigation(true)+'</nav><main class="work">'),'work'):html;
}
