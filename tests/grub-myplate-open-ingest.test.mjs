...
test('MyPlate recipes retain public-domain provenance and fail closed before review',()=>{
  ...
  assert.equal(recipe.source.role,'nutrition_reference_and_localisation_candidate');
  assert.equal(recipe.market.eligible_for_uk_publication,false);
  ...
  assert.ok(recipe.review.blockers.includes('uk_recipe_reauthoring'));
});
