...
test('reported public question traverses real adapter and route without private reads for absent or stale cookies',async()=>{
 for(const cookie of [null,'sst_session=expired','sst_session=revoked']){
  ...
  assert.equal(calls[0].useJourney,false);assert.match(c.nodes.timberResponse.innerHTML,/Reviewed weight loss information/);assert.doesNotMatch(c.nodes.timberResponse.innerHTML,/Psychiatric|survey|401/);assert.equal(c.nodes.timberSubmit.disabled,false);assert(!DB.reads.some(sql=>/user_sessions|member_state|consents|check_ins/.test(sql)));
 }
});
...
test('unrelated authority, filler words, substring and metadata matches cannot become an answer',async()=>{
 ...
 const response=await askTimberRoutes(new Request('https://shiftsometimber.co.uk/v1/ai/chat',{method:'POST',body:JSON.stringify({message:QUESTION,useJourney:false})}),{DB,AI:{run:async()=>{throw Error('must not generate')}}});
