...
function controlledDatabase(mode) {
  ...
  const changed = FIT_EXPANSION_SERVING_AUTHORITY.protected_v1.map(({id}) => ({id,title:'Changed recipe',data_json:JSON.stringify({provenance:{final_v1_acceptance:{accepted:true}}}),review_json:'{"status":"approved"}'})).sort((a,b) => a.id.localeCompare(b.id));
...
test('missing or changed Fit authority returns 503 before the legacy planner or replacement can write', async () => {
  for (const endpoint of ['plan','replace']) for (const mode of ['missing','changed-originals']) {
    const {DB,trace} = controlledDatabase(mode);
    const response = await memberProductV7Routes(new Request(`https://shiftsometimber.co.uk/v1/fit/${endpoint}`,{method:'POST',headers:{Cookie:'sst_session=synthetic-test-session','Content-Type':'application/json'},body:JSON.stringify({days:3,type:'lunch',current_id:'synthetic-current'})}),{DB},{});
...
test('D1, parsing and pagination failures produce a controlled Fit 503 before plan or feedback writes', async () => {
  for (const endpoint of ['plan','replace']) for (const mode of ['database-error','invalid-json','invalid-pagination']) {
    const {DB,trace} = controlledDatabase(mode);
    const response = await memberProductV7Routes(new Request(`https://shiftsometimber.co.uk/v1/fit/${endpoint}`,{method:'POST',headers:{Cookie:'sst_session=synthetic-test-session','Content-Type':'application/json'},body:'{}'}),{DB},{});
    assert.equal(response.status,503,`${endpoint}/${mode}`);
    assert.equal((await response.json()).error,'fit_catalogue_unavailable');
