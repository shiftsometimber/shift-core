...
export function buildGrubExpansionPublication({result,decisions,authorship,humanAcceptance,ownerAcceptance,existingRows,selectedTemplateKeys}) {
  need(result?.summary?.proof === 'GRUB_ADDITIVE_EXPANSION_REVIEW_V1', 'candidate proof required');
  ...
  const ownerPublication = ownerAcceptance?.proof === 'GRUB_OWNER_PUBLICATION_INSTRUCTION_V1';
  if (ownerPublication) {
    need(ownerAcceptance.status === 'authorised' && ownerAcceptance.instruction === 'Publish them all !!!!!!' && ownerAcceptance.actor?.kind === 'human' && ownerAcceptance.actor?.role === 'owner' && ownerAcceptance.actor?.id === 'Matt O’Brien' && Number.isFinite(Date.parse(ownerAcceptance.recorded_at)) && ownerAcceptance.human_editorial_review_claimed === false, 'exact owner publication instruction required');
    need(ownerAcceptance.candidate_sha256 === hash(result) && ownerAcceptance.independent_review_sha256 === hash(decisions) && ownerAcceptance.authorship_sha256 === hash(authorship), 'owner instruction candidate or review changed');
    need(selected.length === 87, 'owner instruction covers the complete prepared batch');
  } else need(humanAcceptance?.proof === 'GRUB_ADDITIVE_HUMAN_ACCEPTANCE_V1', 'exact human second-person acceptance required by SHIFT-MEMBER-HUMANNESS-STANDARD');
  const acceptance = ownerPublication ? ownerAcceptance : humanAcceptance;
  const humanDecisions = uniqueMap(acceptance.families,'template_key','human acceptance');
  const approved = [];
  for (const key of selected) {
    ...
    if (ownerPublication) {
      need(human?.decision === 'AUTHORISE_PUBLICATION' && human.template_digest === family.template_digest && Array.isArray(human.recipes) && same(descendants(human.recipes),descendants(rows)), `owner instruction family mismatch: ${key}`);
    } else {
    ...
    for (const row of rows) {
      ...
      need(!author.author_ids.includes(ownerPublication ? acceptance.actor.id : human.reviewer.id), `independent human reviewer required: ${row.id}`);
  ...
  const reviewDigest = hash({decisions,authorship}), humanDigest = hash(acceptance);
  ...
  const items = approved.map(({row,review,family,human}) => {
    ...
    const item = {id:row.id,contentType:'recipe',title:row.title,version:1,status:'published',data,review:{status:'approved',scope:'exact_recipe_and_family',decision_source:decisions.proof,content_hash:row.content_hash,template_digest:family.template_digest,reviewer:structuredClone(review.reviewer),reviewed_at:review.reviewed_at,scopes:[...review.scopes],author_ids:[...review.author_ids],...(ownerPublication ? {owner_publication:{proof:acceptance.proof,instruction:acceptance.instruction,actor:structuredClone(acceptance.actor),recorded_at:acceptance.recorded_at,template_digest:family.template_digest,human_editorial_review_claimed:false}} : {human_acceptance:{proof:acceptance.proof,template_digest:family.template_digest,reviewer:structuredClone(human.reviewer),reviewed_at:human.reviewed_at,scopes:[...human.scopes]}})}};
    assertPublishableStructuredContent(item);
    return item;
  });
  const servingManifest = {proof:'GRUB_EXPANSION_SERVING_AUTHORITY_V1',status:'approved',release_id:releaseId,review_digest:reviewDigest,human_acceptance_digest:humanDigest,acceptance_kind:ownerPublication?'owner_publication':'human_editorial_review',protected_v1:structuredClone(result.protected_v1),quarantined_ids:sorted(excluded.keys()),additions:items.map(item => ({id:item.id,title:item.title,content_hash:item.review.content_hash,data_sha256:textHash(JSON.stringify(item.data)),review_sha256:textHash(JSON.stringify(item.review))}))};
