...
const publishedAt=row=>parse(row.content_package_json,{}).seo?.datePublished||row.reviewed_at||row.created_at||"";
...
function indexMain(rows){
 const tagged=rows.map(row=>({...row,filters:classifyNewsFilters(row)}));
 ...
 const filters='<form class="newsroom-filters" data-news-filters aria-label="Filter newsroom stories">'+field('medicine','Medicine',medicines,'All medicines')+field('topic','Topic',topics,'All topics')+field('region','Region',[['uk','UK'],['us','US'],['world','World / other']],'All regions')+'<button type="button">Clear filters</button></form><p class="newsroom-filter-results" data-news-results role="status" aria-live="polite">Showing '+rows.length+' stories</p><div class="newsroom-filter-empty" data-news-empty hidden><p>No stories match these filters. Try another combination or clear your selection.</p><button type="button" data-news-clear-empty>Clear filters</button></div>';
 const cards=items=>items.map(row=>{const content=parse(row.content_package_json,{}),seo=content.seo||{},date=publishedAt(row).slice(0,10),tags=row.filters;return '<article class="radar-news-card" data-news-card data-medicine="'+tags.medicines.join(' ')+'" data-topic="'+tags.topics.join(' ')+'" data-region="'+tags.region+'"><p class="eyebrow">'+esc(row.regulator||row.event_type||"Medicines update")+'</p><h2><a href="'+esc(newsUrl(row))+'">'+esc(content.headline||row.headline)+'</a></h2><p>'+esc(content.standfirst||seo.description||content.what_changed||"")+'</p><p class="radar-news-meta">'+esc(row.region||'GLOBAL')+' · '+esc(date)+'</p><p><a href="'+esc(newsUrl(row))+'">Read the evidence-led update →</a></p></article>'}).join("");
 const {uk,international}=partitionNewsRows(tagged);
 return '<main class="page-template template-top-level" id="main-content"><section class="pagehero v6-watermark-host"><div class="wrap"><p class="eyebrow">SHIFT Newsroom</p><h1>SHIFT <span class="accent">Newsroom</span></h1><p>UK health, NHS access and medicine updates, followed by international research worth understanding. Clear sources, evidence limits and what each story means here.</p><p class="radar-news-meta"><a href="/editorial-standards#newsroom">Our editorial process</a> · <a href="/editorial-standards#corrections">Corrections</a></p><p class="radar-news-count">'+uk.length+' UK updates · '+international.length+' international updates.</p></div></section><section class="content"><div class="wrap">'+filters+'<section data-news-group><h2>UK health and access</h2>'+(uk.length?'<div class="radar-news-grid">'+cards(uk)+'</div>':'<p>UK updates are being reviewed. We only publish completed, approved articles.</p>')+'</section><section data-news-group><h2>International research and developments</h2><div class="radar-news-grid">'+cards(international)+'</div></section></div></section></main><script>'+NEWSROOM_FILTER_SCRIPT+'</script>';
}
...
function detailMain(row){
 ...
 const meaning=String(content.why_it_matters_to_uk||'').trim();
 const meaningInBody=meaning&&String(content.article_markdown||'').split(/\n\s*\n/).some(paragraph=>paragraph.trim()===meaning);
 const meaningHtml=meaningInBody?'':'<section><h2>What this means in the UK</h2><p>'+esc(meaning||'Check the cited evidence and current UK guidance.')+'</p></section>';
...
function detailHead(html,row,request){
 ...
 const image=String(seo.image||"/assets/og-default.jpg"),absoluteImage=image.startsWith("http")?image:"https://shiftsometimber.co.uk/"+image.replace(/^\//,""),schema={"@context":"https://schema.org","@type":"Article",headline:title,description,image:[absoluteImage],datePublished:seo.datePublished||publishedAt(row),dateModified:seo.dateModified||row.updated_at||publishedAt(row),author:{"@type":"Organization",name:seo.author||"SHIFT Newsroom"},publisher:{"@type":"Organization",name:"Shift Some Timber",logo:{"@type":"ImageObject",url:"https://shiftsometimber.co.uk/assets/shift-wordmark.png"}},mainEntityOfPage:canonical.href};
...
function newsroomHead(html){
 ...
 html=html.replace(/<title>[\s\S]*?<\/title>/i,'').replace(/<meta\s+name=["']description["'][^>]*>/gi,'').replace(/<link\s+rel=["']canonical["'][^>]*>/gi,'').replace(/<meta\s+(?:property|name)=["'](?:og:|twitter:)[^>]*>/gi,'').replace(/<script\s+type=["']application\/ld\+json["'][^>]*>[\s\S]*?<\/script>/gi,'');
 const schema={'@context':'https://schema.org','@type':'CollectionPage',name:title,description,url:canonical,publisher:{'@type':'Organization',name:'Shift Some Timber',url:'https://shiftsometimber.co.uk/'}};
 return html.replace('</head>','<title>'+title+'</title><meta name="description" content="'+description+'"><link rel="canonical" href="'+canonical+'"><meta property="og:title" content="'+title+'"><meta property="og:description" content="'+description+'"><meta property="og:url" content="'+canonical+'"><meta property="og:type" content="website"><meta property="og:image" content="https://shiftsometimber.co.uk/assets/og-default.jpg"><meta name="twitter:card" content="summary_large_image"><script type="application/ld+json">'+JSON.stringify(schema)+'</script></head>');
}
