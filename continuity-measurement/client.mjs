...
export const continuityExposureRuntime=String.raw`(()=>{
 ...
 async function check(){
  ...
  pending=true;
  try{const response=await fetch('/v1/events',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({event_name:'continuity_today_exposed',surface:'today',properties:{}})});if(response.ok){sent=true;observer.disconnect();document.removeEventListener('visibilitychange',check);window.removeEventListener('hashchange',check)}}catch{}finally{pending=false}
 }
 check();
})();`;
