...
const checks={
  ...
  strengthStatusIsSpecific:/data-strength/.test(product)&&/SELECTED CATALOGUE ITEM/.test(product)&&/Price/.test(product)&&/Availability/.test(product)&&/not a dose recommendation/.test(product),
  clearLockedEnding:/Information ends here/.test(product)&&/No checkout or order action exists/.test(product),
  ...
  aboutAuthority:/SHIFT SOME TIMBER\.\<br\>GET YOUR LIFE BACK/.test(about)&&/independent qualified clinician/.test(about)&&/ordinary blokes/.test(about)&&/Find my treatment route/.test(about),
  aboutServiceLevel:!/Mounjaro|Wegovy|Ozempic|tirzepatide|semaglutide|£\d/i.test(about)
};
