...
test('real push library uses aes128gcm and signed modern VAPID without plaintext health data',async t=>{
 ...
 const headers=new Headers(captured.options.headers);assert.equal(headers.get('content-encoding'),'aes128gcm');assert.equal(headers.get('ttl'),'1800');assert.equal(captured.options.redirect,'manual');
 ...
 let calls=0;
 assert.equal(await sendPwaPush({DB,VAPID_PUBLIC_KEY:publicKey,VAPID_PRIVATE_KEY:priv.d},{endpoint:'https://web.push.apple.com/synthetic-no-network',p256dh:clientKey,auth:Buffer.from(crypto.getRandomValues(new Uint8Array(16))).toString('base64url')},async(_,options)=>{calls++;assert.equal(options.redirect,'manual');return new Response(null,{status:307,headers:{Location:'https://foreign.example.invalid'}})}),'rejected');
