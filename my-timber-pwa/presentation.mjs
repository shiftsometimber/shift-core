...
#pwaReminderFirstRun{box-sizing:border-box;max-width:920px;width:calc(100% - 24px);margin:8px auto 16px;padding:18px 20px;border:1px solid #707762;border-radius:14px;background:#050505;color:#e7e3da}
#pwaReminderFirstRun[hidden]{display:none!important}#pwaReminderFirstRun :is(h2,p){color:#e7e3da!important}#pwaReminderFirstRun h2{font-size:21px;margin:0 0 6px}
#pwaReminderFirstRun p{margin:6px 0 12px}#pwaReminderFirstRun .pwa-actions{display:flex;gap:8px;flex-wrap:wrap}
#pwaReminderFirstRun button{min-height:44px;border:1px solid #707762;border-radius:8px;padding:10px 14px;font:700 16px/1.3 Arial,sans-serif;cursor:pointer}
#pwaReminderYes{background:#e7e3da;color:#050505}#pwaReminderNo{background:transparent;color:#e7e3da}#pwaReminderFirstRun button:focus-visible{outline:3px solid #e7e3da;outline-offset:3px}
...
export const reminderFirstRun=`<section id="pwaReminderFirstRun" hidden aria-labelledby="pwaReminderFirstRunTitle"><h2 id="pwaReminderFirstRunTitle">Want a daily check-in reminder?</h2><p>One quiet nudge at 19:00 UK time. No health details on your lock screen. You can change it later in Settings.</p><div class="pwa-actions"><button type="button" id="pwaReminderYes">Yes, remind me</button><button type="button" id="pwaReminderNo">No thanks</button></div><p id="pwaReminderFirstRunStatus" role="status" aria-live="polite"></p></section>`;
...
export async function withPwa(request,response){
 ...
 if(/^\/member\/dashboard(?:\.html)?\/?$/.test(path)&&!html.includes('id="pwaReminderFirstRun"'))html=html.replace(/(<main\b[^>]*>)/,'$1'+reminderFirstRun);
