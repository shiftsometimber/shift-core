...
export default{async fetch(request,env){
 ...
 if(path==='/__preview/start'&&request.method==='POST'){
  if(request.headers.get('Origin')!==u.origin)return new Response('Same origin required',{status:403});
 ...
 if(['/','/my-timber','/member/dashboard','/member/check-in'].includes(path)){
  const auth=await authenticateMember(request,env);
  if(auth.response)return page('<h1>Try My Timber on your phone.</h1><aside>This is only an install and notification test, not the live My Timber dashboard. It creates an anonymous fictional account on this preview. Do not enter real account details.</aside>'+(u.searchParams.has('finished')?'<p>Your preview account and saved push subscription have been removed.</p>':'')+'<form action="/__preview/start" method="post"><button>Start device test</button></form><p>The test is optional. You choose whether to install and allow notifications. Daily scheduled reminders are disabled here.</p>');
