...
export async function draftEvidencePackage(env,packageId,{model='@cf/meta/llama-3.1-8b-instruct-fast'}={}){
  ...
  if(!Array.isArray(evidence)||!evidence.length)return{ok:false,status:409,error:'source_evidence_required'};
  const prompt=`You are the Shift Evidence Desk drafting assistant. Return JSON only with keys pagePath, contentKey, proposedText. Draft the smallest exact replacement wording supported only by the supplied authoritative evidence. If a change instruction supplies requiredText, proposedText must reproduce it exactly. Do not invent facts, diagnose, prescribe, imply clinical review or add promotional language. Preserve risk qualifiers and urgent instructions. Package risk: ${state.row.risk_lane}. Communications class: ${state.row.communication_class}. Evidence: ${JSON.stringify(evidence)}. Change instructions: ${JSON.stringify(changes)}.`;
  ...
  const requiredText=clean(changes.find(change=>change?.pagePath===pagePath&&change?.contentKey===contentKey)?.requiredText,15000);
  if(requiredText&&proposedText!==requiredText)return{ok:false,status:422,error:'model_copy_not_evidence_locked'};
