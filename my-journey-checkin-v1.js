...
async function getWeekly(request,env,uid){
  ...
  const activity=Object.values(preferences.fitJourney?.entries||{}).filter(x=>x?.status==='done'&&x.recordedOn>=start&&x.recordedOn<=weekEnding);
  prefill.movement.exercisesCompleted=activity.length;
  prefill.movement.activeDays=new Set(activity.map(x=>x.recordedOn)).size;
