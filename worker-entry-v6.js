...
async function memberAsset(env,name,authority){const asset=await env.MEMBER_ASSETS.fetch(new Request(`https://member-assets.local/${name}`,{method:'GET'}));const headers=new Headers(asset.headers);headers.set('Content-Type','application/javascript; charset=utf-8');headers.set('Cache-Control','public, max-age=300, must-revalidate');headers.set('X-Content-Type-Options','nosniff');headers.set('X-Shift-Frontend-Authority',authority);return new Response(asset.body,{status:asset.status,statusText:asset.statusText,headers});}

export default {
  async fetch(request,env,ctx){
    const path=new URL(request.url).pathname;
    if(path==='/api-adapter-v33d.js'&&env.MEMBER_ASSETS)return memberAsset(env,'api-adapter-v33d.js','git:frontend/member/api-adapter-v33d.js');
    if(path==='/progress-units-overlay-v1.js'&&env.MEMBER_ASSETS)return memberAsset(env,'progress-units-overlay-v1.js','git:frontend/member/progress-units-overlay-v1.js');
    if(path==='/member-shell-v33g.js'&&env.MEMBER_ASSETS){const asset=await env.MEMBER_ASSETS.fetch(new Request('https://member-assets.local/member-shell-v33g.js',{method:'GET'}));let source=await asset.text();source+='\n;import("/progress-units-overlay-v1.js?v=1").catch(()=>{});\n';const headers=new Headers(asset.headers);headers.set('Content-Type','application/javascript; charset=utf-8');headers.set('Cache-Control','public, max-age=300, must-revalidate');headers.set('X-Content-Type-Options','nosniff');headers.set('X-Shift-Frontend-Authority','git:frontend/member/member-shell-v33g.js+progress-units-overlay-v1.js');return new Response(source,{status:asset.status,statusText:asset.statusText,headers});}
    ...
    const commissioningIdentity=await handleCommissioningIdentity(request,env,ctx,(req,e,c)=>hq.fetch(req,e,c));if(commissioningIdentity)return withMemberCors(commissioningIdentity,request);
    const emailVerification=await handleEmailVerification(request,env,ctx,(req,e,c)=>hq.fetch(req,e,c));if(emailVerification)return withMemberCors(emailVerification,request);
    const authRecovery=await handleAuthRecovery(request,env,ctx,(req,e,c)=>hq.fetch(req,e,c));if(authRecovery)return withMemberCors(authRecovery,request);
  ...
  async scheduled(controller,env,ctx){const job=Promise.all([runScheduledIntelligence(env),runRadarScheduledScan(env),runKnowledgeFlywheel(env,{limit:1000})]).then(r=>console.log('shift_scheduled_intelligence',JSON.stringify(r))).catch(e=>console.error('shift_scheduled_intelligence_failed',e?.message));if(ctx?.waitUntil)ctx.waitUntil(job);else await job;}
};
