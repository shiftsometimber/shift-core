...
const SHIFT_HEALTH_SEO = {
  "health-mot": ["SHIFT Health MOT | Menâs Home Health Check", "Understand the health markers that matter with a plain-English home-testing route and a clear next step."],
  ...
  "hair-loss": ["Menâs Hair Loss Support | SHIFT Health", "Understand common patterns of male hair loss, realistic expectations and what a legitimate assessment should involve."],
...
async function shiftHealthWithServerSeo(response, request, slug) {
  ...
  html = html
    ...
    .replace('<main class="wrap" data-product></main>', `<main class="wrap" data-product><a class="back" href="/shift-health">â SHIFT Health</a><section class="productHero"><div><small>SHIFT HEALTH</small><h1>${htmlEscape(title.replace(/ \|.*$/, ""))}</h1><p class="job">${htmlEscape(description)}</p><p>This page contains general information, not individual medical advice. The complete interactive route loads below.</p></div></section></main>`);
  const headers = new Headers(response.headers);
  headers.delete("Content-Length");
  headers.set("Cache-Control", "public, max-age=300, must-revalidate");
