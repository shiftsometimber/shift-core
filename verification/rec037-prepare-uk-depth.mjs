...
const batchId='REC-037-UK-archive-20260913';
const authorities=new Map([
 ...
 ['Welsh Government',['www.gov.wales','media.service.gov.wales']],['Scottish Government',['www.gov.scot']],
 ['Department of Health Northern Ireland',['www.health-ni.gov.uk']],
 ['Mental Health UK',['mentalhealth-uk.org']],['UCL',['www.ucl.ac.uk']],['SAMH',['www.samh.org.uk']],['Public Health Scotland',['publichealthscotland.scot']],['NIHR',['www.nihr.ac.uk','www.journalslibrary.nihr.ac.uk']],['University of Oxford',['www.ox.ac.uk']],['University of Bristol',['www.bristol.ac.uk']]
]);
...
for(const article of archiveBatch){
 ...
 assert.match(article.date,/^202[56]-[0-9]{2}(?:-[0-9]{2})?$/);
 ...
 assert.ok(article.body.split(/\s+/).length>=220,article.slug+' too short');
...
assert.equal(archiveBatch.length,28);
if(!process.argv.includes('--remote')){
 console.log('PASS: 28 original UK archive packages; source, date, evidence, relevance and web-only destination checks.');
 process.exit(0);
}
...
for(const article of archiveBatch){
 ...
 const now=new Date().toISOString(), verification={...verifyEvidence(article.evidence),reason:'Primary UK source checked for this archive article; editorial verification is not clinical review.'};
...
fs.writeFileSync('uk-depth-preparation.json',JSON.stringify({batchId,prepared,skipped,beforeCounts,afterCounts,eligibleForReview:eligible},null,2));
