import {RETA_BASE_MAIN,RETA_RECOVERY_CSS} from './reta-critical-styles.mjs';
...
export function repairRetaPresentation(html,path){
 ...
 const originalMain=html.match(/<main\b[\s\S]*?<\/main>/i)?.[0]?.replace(/<figure data-reta-editorial-image[\s\S]*?<\/figure>/,'');
 const knownLayout=originalMain===RETA_BASE_MAIN;
 ...
 // Inline existing rules at their original cascade positions. The exact known
 // article uses its verified subset; changed content falls back to the full CSS.
 // No delayed styles, rewritten URLs or consent-script changes.
