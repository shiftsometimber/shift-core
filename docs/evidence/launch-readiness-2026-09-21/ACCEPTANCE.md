...
## LR-N07 — login completion racing password reset, added before repair

A controlled SQLite test pauses ordinary login after password validation but before its D1 batch, commits password reset, then resumes login. Existing code returned 200 and created a session using the obsolete credential. Require session insertion to conditionally match the password hash, verified-email and current lock state at commit. Remaining success effects must be gated by that inserted session. The legacy login route must use the same guarded implementation. Prove rejected late login creates no active session, fresh new-password login works, and a reset between validation and batch behaves identically in isolated real D1. Preserve PBKDF2, lockout, cookie scope/lifetimes, explicit logout, analytics and current public journeys.
