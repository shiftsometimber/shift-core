...
## LR-N06 — A delayed expired-session response can erase a newer login (confirmed mechanism, high confidence; WR01/02)

Preview 67b9114 returned HTTP 401 at a consent save after ordinary re-login in WebKit desktop. Earlier Chromium failures after reset occurred at a different read. Inspection found both `authenticateMember` and legacy `requireUser` returned an unconditional `Set-Cookie` deletion on an expired/invalid read. A deterministic test captured that rejected old response, applied it after a newer valid cookie, and reproduced loss of authenticated access in both `/v1/member-state` and `/v1/me` paths before the change. This confirms the mechanism; the precise response ordering of the original intermittent incident was not recorded.

The minimal repair returns the same 401 without mutating the cookie. Explicit logout and account-deletion responses still clear it; expiry, revocation, credential checks and duplicate-cookie handling remain enforced. Eighty relevant auth/save/reset/Passport/consent tests pass after repair, including explicit logout. The hosted B1 matrix now obtains actual revoked-session responses from isolated D1 in a separate context, replays their status/body/headers after ordinary new login in the browser, and requires the fresh session to survive. No cookie is manufactured or re-scoped; only the stale response is replayed. This is a release blocker until that controlled replay and the complete final matrix pass. Technical owner Codex; acceptance/operational owner Matt.
