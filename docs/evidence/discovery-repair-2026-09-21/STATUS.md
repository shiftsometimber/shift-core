...
Local acceptance: 241 tests passed, plus the existing final Today, daily front-door and One Shift Brain source gates. Hosted acceptance is pending. Release preservation now recognises only the exact additional paint stylesheet, while continuing to compare all other bytes; missing, duplicate or altered paint repairs fail. The security source gate follows the imported deletion handler and still requires authenticated dispatch, atomic receipt/queue creation and all-session revocation. One existing staging test fixture now retains subdirectories when reading explicit current-source assets; no allowed-path checks were removed.
