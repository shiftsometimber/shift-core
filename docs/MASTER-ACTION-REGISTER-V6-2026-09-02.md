...
| Member rendered acceptance | **RED UNTIL NEW SERIAL JOB PASSES** — G2-011, G2-013, G2-014 and G2-015 previously collided under concurrent production execution. One orchestrated job runs all four lanes serially after a successful Cloudflare production promotion, retains every lane's evidence even after a failure, and enforces one final all-four result. Its first run correctly exposed and led to repair of a missing OIDC workflow allowlist entry. The original workflows are manual diagnostics only. |
