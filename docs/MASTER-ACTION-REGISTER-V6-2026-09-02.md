...
| Member rendered acceptance | **RED UNTIL NEW SERIAL JOB PASSES** — the serial job exposed two contract defects: My Plans opened nonexistent dashboard tabs instead of the real Grub/Fit routes, and the picture proof required an empty history container to be visible. Those are corrected, with one bounded reload added for the observed intermittent authenticated-dashboard readiness race. Production promotion now covers `frontend/member/**` instead of a brittle incomplete asset list. Fresh post-promotion evidence is still required before closure. |
