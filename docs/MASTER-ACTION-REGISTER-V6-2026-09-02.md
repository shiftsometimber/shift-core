...
Static deployment candidate: V1.47 Final Consolidated Closeout (supersedes V1.44–V1.46 checkpoints)
...
| Lounge public chrome | **V1.47 FINAL CANDIDATE — DEPLOYMENT REQUIRED** — all 451 HTML files, shared runtime navigation, About/member chrome and both search indexes use The Lounge and `/lounge`. Only explicit one-way legacy redirects and the stable noindex FAQ slug retain the old route string. Modified assets, service-worker registration and the cache namespace move to `v47`. Package SHA-256: `112caac8e96af2324599b41adf1b6578286f0ba31d35b44d5908a8159ca1fcfa`. |n, but the production render proved shared JavaScript could restore Tap Room links. V1.46 retains the clean HTML/JavaScript and changes every modified shared-asset URL to cache key `v46`, forcing browsers to load the corrected Lounge chrome while retaining explicit redirects. Human visual sign-off remains paused. Package SHA-256: `9358d00cca34f62c88a82071f45fe0b7430c8e5db288fbde4cff14e6c451a1c3`. |nd links from all 451 source HTML files, makes `/lounge` the sole destination, retains explicit legacy redirects, and leaves the homepage ticker-free. Human visual sign-off remains paused. Package SHA-256: `7febe92db37ae14256d582238dce5bcf19d4d4ebe657a68f0914b3ba062abc91`. |
...
| SHIFT AI wire | **V1.47 FINAL CANDIDATE — DEPLOYMENT REQUIRED** — Explore Knowledge, Treatment Centre, `/medicine-news` and member Medicines Watch use one complete current approved wire. Public and member static-library fallbacks are removed; RED, stale, empty or unreachable output is hidden; no item cap exists; homepage remains ticker-free. |n rendering proved the full wire on `/medicine-news`, but also exposed a missing Treatment Centre ticker body and a static-library fallback when the live desk was RED. V1.46 cache-busts and enforces full current approved wire or hidden across Explore Knowledge, Treatment Centre and `/medicine-news`; homepage remains ticker-free. |nowledge and Treatment Centre chrome uses canonical `/v1/radar/ticker`, renders the full approved wire with no item cap, and removes the strip when the desk reports non-current or empty. Production is correctly RED/empty today, so no ticker appears. Homepage remains ticker-free. |
