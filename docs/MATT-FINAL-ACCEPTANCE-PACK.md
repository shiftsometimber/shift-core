...
**Status: FINITE HUMAN TAIL / 5 CATEGORY-A ROWS REMAIN.** The authoritative 57-row matrix is **49 PASS / 5 AMBER / 3 BLOCKED**, with **A=5 / B=0 / C=3**. Keep this as one finite acceptance session where possible; do not reopen already-earned production PASS rows.
...
**G1-001 PASS.** Fresh production commissioning used the genuinely verified connected-inbox member, received the real reset email, consumed the genuine reset token, logged in with the reset password, completed authenticated change-password, logged out and completed a fresh final login. No password or token is retained in evidence.
...
Check: no horizontal scroll; My Shift remains inside the authenticated member experience; navigation opens/closes; notices never intercept intended controls; closed Ask Timber does not enlarge the page; forms/selects feel production-grade; loading/empty/error states are intelligible; back/forward works; keyboard does not obscure critical controls.

### Chrome mobile

Repeat the same critical path. Confirm no browser-specific clipping, pointer interception, stale session or blank state.
...
- **G5-013** — final unassisted/device Dave acceptance; external treatment support remains outside non-clinical V1.
...
The non-clinical RC reaches the freeze line only when the five Category-A rows above are genuinely closed. Then: freeze new product work -> deploy RC -> full regression + Dave + security smoke + Watchtower + routes + accessibility/performance + final device acceptance -> fix release defects only -> **NON-CLINICAL V1 READY**.
