...
| G4-004 | Site/CMS content ingestion into Knowledge Graph not proven automatic | **PASS** | Dedicated unchanged lifecycle gate proves scheduled CMS/approved-document ingestion -> canonical Knowledge Graph provenance, excludes drafts, withdraws graph content when the source is withdrawn, preserves canonical IDs, and reactivates only after re-review/update. |
