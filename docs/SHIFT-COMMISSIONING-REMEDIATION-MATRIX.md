...
| G1-006 | Broken/dead routes and links not centrally detected | **PASS** | M10 production release sweep exhausted same-origin discovery: 418 URLs / 370 HTML pages, zero critical route/asset/blank-page failures, `truncated:false`; the gate now fails partial discovery and retains discovery-parent diagnostics. Evidence: `docs/evidence/2026-08-12-m10-exhaustive-production-sweep.md`. |
| G1-007 | Error handling exposes generic failures | AMBER | Request IDs/contracts exist in Core; exhaustive member/HQ production error-contract sweep is now being added in PR #82, with rendered member-facing failure acceptance still separate. |
...
| G5-009 | Recipes/exercises are hard-coded scaling traps | **PASS** | M07 authenticated production commissioning serves reviewed/published structured V7 Grub/Fit objects, including validated nutrition and approved Fit visual guidance, while preserving durable Nay behaviour; legacy is controlled migration fallback only. Evidence: `docs/evidence/2026-08-12-m07-production-structured-serving.md`. |
