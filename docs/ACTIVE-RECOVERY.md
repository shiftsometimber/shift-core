...
The authoritative original audit at this snapshot is **57 total / 49 PASS / 5 AMBER / 3 BLOCKED / 0 unmapped**. PASS still requires demonstrated journey, retained state where relevant, expected member outcome and retained evidence. A source gate, build, deployment check or green merge is not enough by itself.

## Exact five AMBER rows

### Gate 1 — CLOSED

**G1-001 is PASS.** Genuine connected-inbox reset delivery, real token consumption, reset-password login, authenticated change-password, logout and retained final login are production-proven. Evidence: `docs/evidence/2026-08-14-g1-001-real-password-recovery-pass.md`.
...
The current acceptance pack is reconciled to **A=5 / B=0 / C=3**. Do not ask for already-consumed decisions again. The remaining human work is finite:
...
- G5-013 final unassisted physical-device Dave acceptance.
...
5. Preserve the earned G1-001 recovery PASS; do not generate redundant reset mail absent genuine regression evidence.
