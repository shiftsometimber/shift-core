...
| B08 | Dave release-candidate journey | AMBER — reconciled evidence is **16/20 non-duplicated journey legs (80%)**. Fresh production onboarding proves empty state -> onboarding save -> new authenticated session -> retained personalised `Your Shift starts here` outcome. A one-shot unattended synthetic release gate is now running on main for every automatable Dave lane; real-inbox registration/verification/recovery and partner-dependent treatment support remain outside that synthetic gate. |
