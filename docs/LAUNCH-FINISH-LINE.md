...
| B03 | Core member V1 journeys | Behaviour **9/9 PASS and locked**; full row remains AMBER only for rendered/premium/loading-state evidence. The cross-browser/mobile geometry/routing prerequisite is production-green; G1-008 remains bounded to publishing the canonical static Fit adapter and rerunning loading/empty/success. |
...
| B08 | Dave release-candidate journey | AMBER — reconciled evidence is **16/20 non-duplicated journey legs (80%)**. Fresh production onboarding now proves empty state -> onboarding save -> new authenticated session -> retained personalised `Your Shift starts here` outcome. Remaining human-only legs are registration, verification and account recovery; treatment support remains external BLOCKED. |
...
| ID | Requirement | State / exact closure |
|---|---|---|
| M01 | One Shift premium visual system across public + My Shift | AMBER — public rendered lane is GREEN and G1-009 proves the authenticated Chromium/Firefox/WebKit desktop + 390px member estate has live routing and zero document-root overflow. Remaining work is premium populated/empty/error states, shell/navigation/footer/forms/Progress Picture/member IA parity against the homepage design constitution. |
...
| M06 | Accessibility + performance release check | AMBER — authenticated production evidence now bounds the remaining defects. Keyboard focus, labels, reduced motion, landmarks and zero root overflow are green; demonstrated contrast failures remain on olive primary actions, light-olive eyebrow text and pale form/toggle boundaries. Registration/login also measured materially above the declared API SLO. Fix only demonstrated defects, correct the gradient/filled-control measurement classification and rerun unchanged production acceptance. |
...
| M09 | Proper email verification lifecycle | AMBER — PR #170 deployed the verification-first registration repair and the latest main production commissioning is GREEN. Fresh production registration now sends the genuine verification email without the contradictory pre-verification Welcome. PASS still requires the retained real-inbox token click -> verified login -> post-verification Welcome -> logout -> retained final login sequence. |
| M10 | Whole-estate routes/links/errors release sweep | AMBER — exhaustive routing is green, G1-007 rendered failure-state acceptance is PASS, and G1-009 cross-browser/mobile authenticated routing/geometry is PASS. Explicit loading/empty/success acceptance remains bounded to the G1-008 static frontend publish/rerun. |
...
| M12 | Fit catalogue/session breadth and visual guidance | AMBER — **2,500 authored**; all **26 V1 canonical coaching families** are systemically repaired and cover **1,326 technically eligible descendants**. The 12-week simulator yields 180/180 unique prescribed objects, all 26 families used and no consecutive canonical repeat. Rejected legacy schematic artwork is not launch art. Closure requires genuine premium START -> MOVE -> FINISH anatomical/member-comprehension/domain PASS visuals, propagation/publication and production serving. |
| M13 | Whole-person Progress + proper units | AMBER — coherent story + controlled stone/lb/kg and metric experience. Two bounded live-client defects are tracked: stone rounding can display impossible 14 lb remainder, and saved-photo waist readback ignores the member's inches preference. |
...
**Fit:** structured authored universe **2,500** = 2,468 industrial + 32 existing structured. Industrial schema-valid **2,468**. The V1 launch conversion is **26 canonical movement decisions / 1,326 technically eligible descendants** with repaired human coaching guidance. Rejected legacy schematic artwork is excluded from final launch acceptance. Genuine premium three-state visuals remain the major unfinished production item.
...
## Current swarm
Gate 1: finish genuine verification/recovery token lifecycles and publish the canonical static Fit adapter before rerunning loading/empty/success acceptance. In parallel: premium member parity, Progress units/story, measured accessibility/contrast and auth critical-path performance, product analytics, the finite Grub decision review, and 26-family Fit premium visual production. Human/device and external boundaries do not queue non-blocked work.
