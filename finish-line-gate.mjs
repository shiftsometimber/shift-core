...
must(counts.PASS===21,`original matrix PASS count is 21 (found ${counts.PASS||0})`);
must(counts.AMBER===33,`original matrix AMBER count is 33 (found ${counts.AMBER||0})`);
must(counts.BLOCKED===3,`original matrix BLOCKED count is 3 (found ${counts.BLOCKED||0})`);
must(matrix.includes('PASS rows: 21. AMBER rows: 33. BLOCKED rows: 3.'),'matrix reconciliation summary matches enforced counts');
must(launchFinish.includes('21 PASS / 33 AMBER / 3 BLOCKED'),'launch board scoreboard matches enforced counts');
