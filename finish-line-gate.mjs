...
must(counts.PASS===30,`original matrix PASS count is 30 (found ${counts.PASS||0})`);
must(counts.AMBER===24,`original matrix AMBER count is 24 (found ${counts.AMBER||0})`);
must(counts.BLOCKED===3,`original matrix BLOCKED count is 3 (found ${counts.BLOCKED||0})`);
must(matrix.includes('PASS rows: 30. AMBER rows: 24. BLOCKED rows: 3.'),'matrix reconciliation summary matches enforced counts');
must(launchFinish.includes('30 PASS / 24 AMBER / 3 BLOCKED'),'launch board scoreboard matches enforced counts');
