...
must(counts.PASS===27,`original matrix PASS count is 27 (found ${counts.PASS||0})`);
must(counts.AMBER===27,`original matrix AMBER count is 27 (found ${counts.AMBER||0})`);
must(counts.BLOCKED===3,`original matrix BLOCKED count is 3 (found ${counts.BLOCKED||0})`);
must(matrix.includes('PASS rows: 27. AMBER rows: 27. BLOCKED rows: 3.'),'matrix reconciliation summary matches enforced counts');
must(launchFinish.includes('27 PASS / 27 AMBER / 3 BLOCKED'),'launch board scoreboard matches enforced counts');
