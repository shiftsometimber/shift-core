...
must(counts.PASS===45,`original matrix PASS count is 45 (found ${counts.PASS||0})`);
must(counts.AMBER===9,`original matrix AMBER count is 9 (found ${counts.AMBER||0})`);
must(counts.BLOCKED===3,`original matrix BLOCKED count is 3 (found ${counts.BLOCKED||0})`);
must(matrix.includes('PASS rows: 45. AMBER rows: 9. BLOCKED rows: 3.'),'matrix reconciliation summary matches enforced counts');
must(launchFinish.includes('45 PASS / 9 AMBER / 3 BLOCKED'),'launch board scoreboard matches enforced counts');
