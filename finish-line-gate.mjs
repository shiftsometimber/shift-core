...
must(counts.PASS===29,`original matrix PASS count is 29 (found ${counts.PASS||0})`);
must(counts.AMBER===25,`original matrix AMBER count is 25 (found ${counts.AMBER||0})`);
must(counts.BLOCKED===3,`original matrix BLOCKED count is 3 (found ${counts.BLOCKED||0})`);
must(matrix.includes('PASS rows: 29. AMBER rows: 25. BLOCKED rows: 3.'),'matrix reconciliation summary matches enforced counts');
must(launchFinish.includes('29 PASS / 25 AMBER / 3 BLOCKED'),'launch board scoreboard matches enforced counts');
