...
must(counts.PASS===44,`original matrix PASS count is 44 (found ${counts.PASS||0})`);
must(counts.AMBER===10,`original matrix AMBER count is 10 (found ${counts.AMBER||0})`);
must(counts.BLOCKED===3,`original matrix BLOCKED count is 3 (found ${counts.BLOCKED||0})`);
must(matrix.includes('PASS rows: 44. AMBER rows: 10. BLOCKED rows: 3.'),'matrix reconciliation summary matches enforced counts');
must(launchFinish.includes('44 PASS / 10 AMBER / 3 BLOCKED'),'launch board scoreboard matches enforced counts');
