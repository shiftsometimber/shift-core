...
must(counts.PASS===37,`original matrix PASS count is 37 (found ${counts.PASS||0})`);
must(counts.AMBER===17,`original matrix AMBER count is 17 (found ${counts.AMBER||0})`);
must(counts.BLOCKED===3,`original matrix BLOCKED count is 3 (found ${counts.BLOCKED||0})`);
must(matrix.includes('PASS rows: 37. AMBER rows: 17. BLOCKED rows: 3.'),'matrix reconciliation summary matches enforced counts');
must(launchFinish.includes('37 PASS / 17 AMBER / 3 BLOCKED'),'launch board scoreboard matches enforced counts');
