...
must(counts.PASS===43,`original matrix PASS count is 43 (found ${counts.PASS||0})`);
must(counts.AMBER===11,`original matrix AMBER count is 11 (found ${counts.AMBER||0})`);
must(counts.BLOCKED===3,`original matrix BLOCKED count is 3 (found ${counts.BLOCKED||0})`);
must(matrix.includes('PASS rows: 43. AMBER rows: 11. BLOCKED rows: 3.'),'matrix reconciliation summary matches enforced counts');
must(launchFinish.includes('43 PASS / 11 AMBER / 3 BLOCKED'),'launch board scoreboard matches enforced counts');
