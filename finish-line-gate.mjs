...
must(counts.PASS===16,`original matrix PASS count is 16 (found ${counts.PASS||0})`);
must(counts.AMBER===38,`original matrix AMBER count is 38 (found ${counts.AMBER||0})`);
must(counts.BLOCKED===3,`original matrix BLOCKED count is 3 (found ${counts.BLOCKED||0})`);
must(matrix.includes('PASS rows: 16. AMBER rows: 38. BLOCKED rows: 3.'),'matrix reconciliation summary matches enforced counts');
must(launchFinish.includes('16 PASS / 38 AMBER / 3 BLOCKED'),'launch board scoreboard matches enforced counts');
