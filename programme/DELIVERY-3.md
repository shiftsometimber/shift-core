...
The Node SQLite adapter proves the local aggregate logic only. A further six contract checks now pass in actual workerd 1.20260911.1 with local Miniflare D1 emulation, at the unchanged 2026-08-09 compatibility date: 12 competing writes (one success, eleven 409s), concurrent duplicate/retry, conflicting operation ID, stale acceptance, ownership and real session expiry. These results still do not establish remote Cloudflare D1 behaviour. See `evidence/workerd-d1-contract.json`. Its scheduling and SQLite locking do not establish Cloudflare Worker or D1 behaviour. The candidate storage adapter targets D1 through a separate `PROGRAMME_DB` binding. KV or Durable Objects would be a different implementation and require their own contract proof.
...
Private entry/return integration; compatibility with existing free tools and saved data; reviewed Fit and four-week/continuity content; outstanding accessibility/browser checks; account provisioning/expiry and service-desk/export/deletion/retention arrangements. This follow-up does not silently close them.
