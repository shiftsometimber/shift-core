...
const v7=fs.readFileSync('member-product-v7.js','utf8'),entry=fs.readFileSync('worker-entry-v6.js','utf8');
...
assert.ok(entry.includes("import {memberProductV7Routes} from './member-product-v7.js'"));assert.ok(entry.includes('await memberProductV7Routes'));
console.log(JSON.stringify({published:{recipes:recipes.length,exercises:exercises.length},recipe:{id:recipe.id,nutrition:recipe.data.nutrition.status,dataset:recipe.data.nutrition.dataset_version},exerciseIds:exercises.map(x=>x.id),runtime:{structuredPreferred:true,legacyFallbackControlled:true,naysPreserved:true}},null,2));
console.log('PASS M07 commissioning-floor structured content is review-gated, publishable, queryable and wired as preferred Grub/Fit member runtime authority with controlled legacy fallback.');
