...
export function radarFreshnessState({lastScan,lastEvent,lastPublication,lastTickerItem,failures=0,scanHealthy=true},now=Date.now()){
 ...
 const reasons=[];if(!scanHealthy)reasons.push({code:'scan_incomplete',level:'RED'});if(failures>0)reasons.push({code:'publication_failures',level:'RED',count:failures});
