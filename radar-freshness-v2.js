...
export function radarFreshnessState({lastScan,lastEvent,lastPublication,lastTickerItem,failures=0,scanHealthy=true,sourceReviews=0},now=Date.now()){
 ...
 const reasons=[];if(sourceReviews>0)reasons.push({code:'source_review_required',level:'RED',count:sourceReviews});if(!scanHealthy)reasons.push({code:'scan_incomplete',level:'RED'});if(failures>0)reasons.push({code:'publication_failures',level:'RED',count:failures});
...
export async function readRadarFreshness(DB){const first=async sql=>{try{return await DB.prepare(sql).first()}catch{return null}},count=async sql=>{try{return Number((await DB.prepare(sql).first())?.c||0)}catch{return 0}};const [scan,event,pub,ticker,failures,sourceReviews]=await Promise.all([first(`SELECT created_at,detail_json FROM radar_audit WHERE action='scan' ORDER BY id DESC LIMIT 1`),first(`SELECT updated_at FROM radar_events ORDER BY updated_at DESC LIMIT 1`),first(`SELECT COALESCE(completed_at,created_at) at FROM radar_publication_jobs WHERE status IN ('complete','completed') ORDER BY COALESCE(completed_at,created_at) DESC LIMIT 1`),first(`SELECT COALESCE(json_extract(content_package_json,'$.seo.datePublished'),reviewed_at,updated_at) at FROM radar_events WHERE status='published' AND EXISTS (SELECT 1 FROM json_each(json_extract(content_package_json,'$.destinations')) WHERE value IN ('ticker_knowledge','ticker_treatments')) ORDER BY at DESC LIMIT 1`),count(`SELECT COUNT(*) c FROM radar_publication_jobs WHERE status='failed' OR (error_text IS NOT NULL AND status!='cancelled')`),pendingSourceChangeMap(DB).then(x=>x.size).catch(()=>1)]);let detail={};try{detail=JSON.parse(scan?.detail_json||'{}')}catch{}return radarFreshnessState({scanHealthy:!scan||(detail.coverage?.complete===true&&detail.tier_one_healthy===true),lastScan:scan?.created_at,lastEvent:event?.updated_at,lastPublication:pub?.at,lastTickerItem:ticker?.at,failures,sourceReviews});}
