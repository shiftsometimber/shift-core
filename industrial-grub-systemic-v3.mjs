...
export function grams(amount,item){const s=norm(amount).toLowerCase();const g=s.match(/(\d+(?:\.\d+)?)\s*g\b/);if(g)return Number(g[1]);const ml=s.match(/(\d+(?:\.\d+)?)\s*ml\b/);if(ml)return Number(ml[1])*(item==='olive oil'?0.92:1);const tsp=s.match(/^(\d+(?:\.\d+)?|\d+\/\d+)\s*tsp$/);if(tsp){const n=fractionNumber(tsp[1]);if(n>0&&item==='lemon juice')return n*5;if(n>0&&TSP_G[item])return n*TSP_G[item]}if(/portion/.test(s)&&PORTION_G[item])return PORTION_G[item];if(/^\d+(?:\.\d+)?$/.test(s)&&EACH_G[item])return Number(s)*EACH_G[item];return null}
