...
const checks=[
 ...
 ['identity and ordinary-bloke prompt guard',route.includes('Identity lock is the highest priority')&&route.includes('ordinary bloke')],
 ['model drift excluded',['bodybuilder','mannequin','Lego'].every(x=>route.includes(x))&&(route.includes('fashion model')||route.includes('fashion/fitness model'))&&(route.includes('six pack')||route.includes('six-pack'))],
 ...
 ['member shell loads creator',shell.includes("SHIFT_ME_API_SRC='/shift-me-api-v1.js?v=1'")&&shell.includes('ensureShiftMePremium()')],
 ['creator performs real photo render',creator.includes('type="file"')&&creator.includes('renderShiftMe(file,appearance(panel))')],
 ['creator performs controlled rerender',creator.includes('rerenderShiftMe(appearance(panel))')],
