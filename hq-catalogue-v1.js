...
export async function hqCatalogueRoutes(request, env, ctx) {
  ...
  if (method === "PATCH" && variantEdit) {
    ...
    await env.DB.batch([
      env.DB.prepare(
        "UPDATE medicine_variants SET cost_pence=?,selling_price_pence=?,target_margin_bps=?,status=?,updated_at=? WHERE id=?",
