...
async function ensureSchema(DB) {
  ...
  await DB.prepare(`UPDATE medicine_variants SET status='archived',updated_at=CURRENT_TIMESTAMP WHERE medicine_id IN (SELECT id FROM medicine_products WHERE lower(name)='orlistat') AND trim(lower(strength_label)) IN ('42','84','168')`).run();
}
