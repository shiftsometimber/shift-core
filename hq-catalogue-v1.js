...
export function defaultSellingPrice(costPence) {
  return sellingPriceFromMargin(costPence, 60);
}
export function sellingPriceFromMargin(costPence, grossMarginPercent) {
  const cost = Math.max(0, Math.round(Number(costPence) || 0));
  const margin = Number(grossMarginPercent);
  if (!cost || !Number.isFinite(margin) || margin <= 0 || margin >= 95)
    throw new RangeError("Gross margin must be greater than 0% and less than 95%.");
...
async function ensureSchema(DB) {
  await DB.batch([
    ...
    DB.prepare(
      `CREATE TABLE IF NOT EXISTS medicine_variant_price_history (id INTEGER PRIMARY KEY AUTOINCREMENT,variant_id INTEGER NOT NULL,cost_pence INTEGER NOT NULL,target_margin_bps INTEGER NOT NULL,selling_price_pence INTEGER NOT NULL,changed_by INTEGER,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(variant_id) REFERENCES medicine_variants(id))`,
    ),
  ]);
  try {
    await DB.prepare(`ALTER TABLE medicine_variants ADD COLUMN target_margin_bps INTEGER NOT NULL DEFAULT 6000`).run();
  } catch {}
...
export async function hqCatalogueRoutes(request, env, ctx) {
      ...
      medicines: medicines.map((m) => ({
        ...
        variants: variants
          ...
          .map((v) => ({
            ...
            targetGrossMarginPercent: Number(v.target_margin_bps || 6000) / 100,
  ...
  if (method === "POST" && variant) {
    const b = await request.json().catch(() => ({})),
      ...
      requestedMargin = Number(b.grossMarginPercent ?? 60),
      targetMarginBps = Math.round(requestedMargin * 100),
      selling = Number.isFinite(requestedMargin) && requestedMargin > 0 && requestedMargin < 95 && cost > 0
        ? sellingPriceFromMargin(cost, requestedMargin)
        : NaN,
      status = clean(b.status, 30) || "out_of_stock",
      ...
      return json(
        {
          ...
          message:
            "Enter a positive cost and a gross margin greater than 0% and less than 95%.",
        },
        400,
      );
    ...
    await env.DB.prepare(
      `INSERT INTO medicine_variants(medicine_id,strength_label,cost_pence,selling_price_pence,target_margin_bps,status,sort_order,created_at,updated_at) VALUES(?,?,?,?,?,?,0,?,?) ON CONFLICT(medicine_id,strength_label) DO UPDATE SET cost_pence=excluded.cost_pence,selling_price_pence=excluded.selling_price_pence,target_margin_bps=excluded.target_margin_bps,status=excluded.status,updated_at=excluded.updated_at`,
    )
      .bind(medicineId, strength, cost, selling, targetMarginBps, effective, stamp, stamp)
      .run();
    ...
    await env.DB.prepare(
      ...
      .run();
    await env.DB.prepare(
      `INSERT INTO medicine_variant_price_history(variant_id,cost_pence,target_margin_bps,selling_price_pence,changed_by,created_at) VALUES(?,?,?,?,?,?)`,
    ).bind(saved.id,cost,targetMarginBps,selling,auth.user.id,stamp).run();
    await audit(
      env,
      auth.user,
    ...
    return json(
      {
        ...
        stockOnHand: stock,
        status: effective,
        targetGrossMarginPercent: requestedMargin,
        ...calculateMargin(cost, selling),
      },
      201,
    );
  ...
  if (method === "PATCH" && variantEdit) {
    ...
    const b = await request.json().catch(() => ({})),
      ...
      requestedMargin = Number(b.grossMarginPercent ?? Number(current.target_margin_bps || 6000) / 100),
      targetMarginBps = Math.round(requestedMargin * 100),
      selling = Number.isFinite(requestedMargin) && requestedMargin > 0 && requestedMargin < 95 && cost > 0
        ? sellingPriceFromMargin(cost, requestedMargin)
        : NaN,
      ...
      return json(
        {
          ...
          error: "invalid_medicine_variant",
          message:
            "Cost must be positive, gross margin must be greater than 0% and less than 95%, and stock must be a whole number of zero or more.",
        },
        400,
      );
    const effective =
    ...
    await env.DB.prepare(
      `INSERT INTO medicine_variant_price_history(variant_id,cost_pence,target_margin_bps,selling_price_pence,changed_by,created_at) VALUES(?,?,?,?,?,?)`,
    ).bind(id,cost,targetMarginBps,selling,auth.user.id,stamp).run();
    await audit(
      env,
      auth.user,
