...
(function(){
  ...
  async function withTimeout(task){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),180000);try{return await task(controller.signal)}catch(err){if(err&&err.name==='AbortError'){const e=new Error('Shift Me took too long to render. Please try again.');e.code='timeout';throw e}throw err}finally{clearTimeout(timer)}}
