(()=>{'use strict';
  ...
  const doseNext=next.cloneNode(false);doseNext.removeAttribute('data-op-next');doseNext.dataset.patientPay='dose';doseNext.style.width='100%';
  const paymentButtons=[doseNext,next];
  ...
  function sync(){if(!enabled)return;const variant=chosen();for(const button of paymentButtons){button.disabled=!variant;button.textContent=variant?'Continue to payment →':'No stock available today';button.onclick=()=>{const selected=chosen();if(selected)location.assign(`/treatment-checkout?variant=${encodeURIComponent(selected.id)}`)}}notice.textContent='After successful payment, your patient details, health questions, GP details and supporting photos open automatically. Treatment still requires clinical approval.';const stock=$('[data-stock-status]');if(stock)stock.textContent=variant?'Available · clinical assessment after payment':'No stock available today';const boundary=$('.op-spec-boundary');if(boundary)boundary.textContent='Information for comparison, not a treatment recommendation. An appropriately regulated prescribing service confirms suitability, medicine and dose after clinical assessment.'+(variant?'':' No stock available today; ordering remains closed.');const context=$('[data-stage-context]');if(context&&$('input[name="treatment-stage"]:checked')?.value!=='new')context.textContent='Choose the strength you currently use. After payment, the pharmacy reviews your treatment history and evidence before confirming suitability.';}
  async function boot(){try{const r=await fetch('/v1/catalogue/medicines',{credentials:'omit',cache:'no-store'}),body=await r.json();if(!r.ok||body.intakeFlow!=='shift_v2')return;products=body.products||[];enabled=true;
    ...
    all('.op-bmi-gate,.op-life-priorities,.op-ready-checklist').forEach(section=>{section.hidden=true;section.style.display='none';section.querySelectorAll('input,select,textarea,button').forEach(control=>control.disabled=true)});
    ...
    const doseActions=document.createElement('div');doseActions.className='op-controls';doseActions.style.marginTop='12px';doseActions.style.marginBottom='16px';doseActions.append(doseNext);dose.after(doseActions);
    ...
    sync();
  }catch{for(const button of paymentButtons){button.disabled=true;button.textContent='Ordering unavailable'}}}
  boot();
})();
