...
(function(){
  ...
  window.SST_API={version:'3.3I',connected:()=>true,health,
    register:data=>request('/auth/register',{method:'POST',body:JSON.stringify({...data,acquisition:window.SSTAcquisition?.registration()||null})}).then(r=>{window.SSTAcquisition?.consumed();return r}).catch(e=>{if(e.code==='email_in_use')window.SSTAcquisition?.consumed();throw e}),login:data=>request('/auth/login',{method:'POST',body:JSON.stringify(data)}).then(r=>{window.SSTAcquisition?.consumed();return r}),logout:()=>{window.SSTAcquisition?.consumed();return request('/auth/logout',{method:'POST'})},requestPasswordReset:data=>request('/auth/request-password-reset',{method:'POST',body:JSON.stringify(data)}),resendVerification:data=>request('/auth/resend-verification',{method:'POST',body:JSON.stringify(data)}),
    getMe:()=>request('/me').then(r=>{window.SSTAcquisition?.consumed();return r}),getProfile:()=>request('/profile'),saveProfile:data=>request('/profile',{method:'PATCH',body:JSON.stringify(data)}),getMemberState,saveMemberState,getMyJourney:()=>request('/journey'),saveMyJourney:data=>request('/journey',{method:'PATCH',body:JSON.stringify(data),timeout:PERSISTENCE_TIMEOUT}),deleteMyJourney:()=>request('/journey',{method:'DELETE',timeout:PERSISTENCE_TIMEOUT}),getJourneyExport:()=>request('/journey/export',{timeout:PERSISTENCE_TIMEOUT}),getJourneyCheckIn:()=>request('/journey/weekly-check-in'),saveJourneyCheckIn:data=>request('/journey/weekly-check-in',{method:'POST',body:JSON.stringify(data),timeout:PERSISTENCE_TIMEOUT}),getJourneyTrends:()=>request('/journey/trends'),
    getTreatmentOrders:()=>request('/treatment/orders'),confirmTreatmentJourneySetup:()=>request('/treatment/journey-setup-complete',{method:'POST',body:'{}'}),
    getPenDay:()=>request('/pen-day',{headers:todayHeaders()}),savePenDay:data=>request('/pen-day',{method:'POST',headers:todayHeaders(),body:JSON.stringify(data||{})}),trackEvent:data=>request('/events',{method:'POST',body:JSON.stringify(data||{})}),
    getProgress:()=>request('/progress'),saveProgress:data=>request('/progress',{method:'POST',body:JSON.stringify(data)}),getMots:()=>request('/health-mot'),saveMot:data=>request('/health-mot',{method:'POST',body:JSON.stringify(data)}),saveHealthInterest:slug=>request('/health-passport/interest',{method:'POST',body:JSON.stringify({slug})}),getCheckIns:()=>request('/check-ins'),saveCheckIn:data=>request('/check-ins',{method:'POST',body:JSON.stringify(data)}),
