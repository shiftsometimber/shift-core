"""Preview real captured public shells with candidate article content.
No account access, form submission or third-party analytics delivery.
"""
 ...
 async with async_playwright() as p:
  ...
  for width in [390,1440]:
   for item in manifest['articles']:
    ctx=await browser.new_context(viewport={'width':width,'height':900},service_workers='block')
    errors=[]
    async def route(r):
     u=urlparse(r.request.url)
     if u.hostname not in ['shiftsometimber.co.uk','www.shiftsometimber.co.uk']:
      blocked.append({'host':u.hostname,'type':r.request.resource_type})
      await r.fulfill(status=200,content_type='application/javascript',body='/* third-party delivery disabled in preview */');return
     if r.request.resource_type=='document':
      if u.path.rstrip('/')!=item['path']:
     ...
     if r.request.method!='GET':
      blocked.append({'path':u.path,'method':r.request.method});await r.fulfill(status=403,content_type='application/json',body='{}');return
     if u.path.startswith('/v1/') and u.path!='/v1/radar/ticker':
      await r.fulfill(status=401,content_type='application/json',body='{"error":"unauthorised_preview"}');return
     if len(cache)>180:raise RuntimeError('Bounded public asset capture exceeded')
     if r.request.url not in cache:
      response=await ctx.request.get(r.request.url,timeout=25000)
      cache[r.request.url]=(response.status,response.headers.get('content-type','application/octet-stream'),await response.body())
     status,typ,body=cache[r.request.url]
     await r.fulfill(status=status,content_type=typ,body=body)
    ...
    try:
     ...
     await page.locator('article.fa-guide h1').wait_for()
     ...
     await page.locator('.fa-toc a[href="#sources"]').click()
     target_visible=await page.locator('#sources').is_visible()
     await page.evaluate('window.scrollTo(0,0)');await page.screenshot(path=str(OUT/(str(item['index'])+'-'+str(width)+'.png')),full_page=True)
     passed=measurements['h1']==1 and measurements['articleVisible'] and measurements['scrollWidth']<=width+1 and not measurements['contrastFailures'] and not measurements['duplicateIds'] and target_visible and not errors
     ...
     result['cases'].append({'path':item['path'],'width':width,'pass':False,'error':str(e),'pageErrors':errors})
    finally:await ctx.close()
  # JavaScript is not required to read any of these five articles.
  for item in manifest['articles']:
  ...
  await browser.close()
 result['assetCount']=len(cache);result['blockedThirdPartyOrWriteRequests']=blocked
 result['pass']=len(result['cases'])==15 and all(x['pass'] for x in result['cases'])
