...
DATE='2026-09-23'
EXPECTED={GUIDE:'97c5b059f4bebea8f308b531f0f3efc080afe9b17d42afe709ad25dffd5bda05','explore-knowledge.html':'d6a9ad79ec08bfe5d17cc27795b5ca4e4b7f244ccd7add4b07259bb03b9491d2','glp1-knowledge-centre.html':'608d8d850e8410fde3f6e1e843d375115d6d1a5b4be93423c1b91818d3ed032d','sitemap.xml':'4d9e6c1476838a4deb0aca633aeb6d33ece5f40eea8c80c4fd8c37acd562e8f5'}
...
SALES_BRIDGE=re.compile(r'<script\b[^>]*\bsrc=[\"\']/assets/shift-service-bridge-v1\.js\?v=2[\"\'][^>]*>\s*</script>',re.I)
...
def entry(name,data):return {'path':name,'sha256':digest(data),'bytes':len(data)}
def once(source,pattern,replacement):
 ...
 return value
def metadata(source,key,value):
 p=re.compile(r'<meta\b(?=[^>]*\b(?:name|property)=[\"\']'+re.escape(key)+r'[\"\'])[^>]*>',re.I)
 attr='property' if key.startswith('og:') else 'name';tag=f'<meta {attr}="{key}" content="{html.escape(value,quote=True)}">'
...
def rewrite_guide(old,main):
 rewrite_source_guard(old)
 ...
 # Only this guide stops loading the script that dynamically reinstates sales copy.
 # Other pages, the bridge asset itself and every other behaviour script are untouched.
 new=once(new,SALES_BRIDGE,'')
 ...
 new=SCHEMA.sub(schema,new);assert counts=={'removed_faq':2,'article':1,'medical_webpage':1},counts
 faq_section=re.search(r'<section\b[^>]*id="faq"[^>]*>([\s\S]*?)</section>',main).group(1)
 pairs=re.findall(r'<h3>(.*?)</h3><p>(.*?)</p>',faq_section,re.S);assert len(pairs)==6
 ...
 assert scripts(SALES_BRIDGE.sub('',old))==scripts(new),'unrelated script changed'
 before_schema=[json.loads(m.group(1)) for m in SCHEMA.finditer(old)];after_schema=[json.loads(m.group(1)) for m in SCHEMA.finditer(new)]
 ...
 return new,{'schema':counts,'faq_questions':len(pairs),'legacy_entity_dates_preserved':original_dates,'modified':DATE,'header_drawer_footer_unchanged':True,'only_script_change':'Remove this guide inclusion of shift-service-bridge-v1.js?v=2; all other scripts byte-identical','source_drift_rejected':True}
def check_guide(source,main):
 ...
 ids=[a['id'] for t,a in main_tags if 'id' in a];assert len(ids)==len(set(ids));assert ANCHORS.issubset(ids)
 ...
 return {'one_h1':True,'canonical':URL,'preserved_section_anchors':len(ANCHORS),'all_internal_fragments_resolve':True,'no_article_sales_routes':True,'all_images_have_alt_attributes':True,'word_count':len(re.sub('<[^>]*>',' ',main).split())}
def build(pages_root,out):
 ...
 base=json.loads(gzip.decompress(raw));assert base['source_fingerprint']==BASE_FP;assert len(base['files'])==877
 ...
 updates[GUIDE],checks['editorial']=rewrite_guide(base['overrides'][GUIDE],main);checks['guide']=check_guide(updates[GUIDE],main)
 # The old pillar container is hidden by the existing Knowledge presentation.
 # Move only the Reta reading link to a visible main-child slot, rather than
 # re-enabling unrelated hidden panels or leaving duplicate Reta anchors.
 knowledge=once(base['overrides']['explore-knowledge.html'],r'<div class="topic-hub"><small>Current-status guide</small><h3>Retatrutide</h3>.*?</div>','')
 reading='<aside data-retatrutide-discovery aria-label="Research reading" style="max-width:1120px;margin:16px auto;padding:14px 20px;color:#E7E3DA;border-block:1px solid #707762;line-height:1.5"><strong>Research explained:</strong> <a href="/guides/retatrutide-uk-guide" style="color:#E7E3DA;text-decoration:underline;text-underline-offset:3px">Retatrutide (Reta) UK guide — status, trial evidence and safety</a>. Information, not an offer of treatment.</aside>'
 assert knowledge.count('<div class="standard-layout">')==1
 updates['explore-knowledge.html']=knowledge.replace('<div class="standard-layout">',reading+'<div class="standard-layout">',1)
 ...
 for name in ['explore-knowledge.html','glp1-knowledge-centre.html']:assert updates[name].count('href="/guides/retatrutide-uk-guide"')==1,name+': duplicate discovery link'
 ...
 assert all(entries[n]==original[n] for n in original if n not in updates);assert set(entries)==set(original)
